...
contract AsunaTron is TRC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public price = 400000000;
    uint256 internal maxMintAmount = 10;
    string internal baseURI;

    constructor(string memory newBaseURI) TRC721("AsunaTron", "AST") {
        baseURI = newBaseURI;
    ...
    function reserveMint(address _to, uint256 qty) public onlyOwner {
        require(qty > 0, "Wrong qty");
        require(
            _tokenIdTracker.current() + qty < maxSupply,
            "Exceeds maxSupply"
        );
