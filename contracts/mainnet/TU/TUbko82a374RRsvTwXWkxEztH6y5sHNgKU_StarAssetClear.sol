...
contract StarAssetClear is Common{
    ...
    // star提币地址
    //address internal fb_draw_address = address(0x4154a5ed7302ce39b719f9daac33d3bb8c186290fa);
    ...
    constructor() public {
        fBankPool = StarPool(address(0x415686b9b6c0d202569709bdb49ea3fe6412aac846));
		starRelation = starRelationIn(address(0x417d0bb9f85bc3120be56da77dc91a53005e62e50b));
    }

    // 用户提币
    function drawCoin(uint amount) public payable returns (uint) {
        require(msg.value >= 15 * 10 ** 6, "fee is not enough");
        fBankPool.subAsset(msg.sender, amount);
    ...
    function getAsset() public payable returns (address[] memory, uint[] memory, uint[] memory) {
       (,,,, uint256 _calp, uint256 preTotalCalp, uint256 profitAsset, uint256 releaseProfitAsset, uint256 signTime) = fBankPool.GetUserInfo(msg.sender);
        ...
        if (_calp > 0 || preTotalCalp > 0) {
            total = totalAmount * (preTotalCalp + _calp * getDays) / (totalCalp + fBankPool.getTotal());
        ...
        fBankPool.assetClear(total, addressList, amountList, assetTypeList);
    ...
    function setAsset(address[] memory _addressList, uint[] memory _amountList, uint[] memory _assetTypeList) onlyAdmin public returns (uint) {
        return fBankPool.assetClear(0, _addressList, _amountList, _assetTypeList);
    }
...
contract StarPool is StarStorage {
    ...
    function buyPackage(uint index, address contractAddress, uint packageType) public payable returns (uint) {
		...
		// reset can only star 0225 
        if (packageType == 1 && openStar) {
            TRC20 token = TRC20(contractAddress);
            ...
            assert(token.transferFrom(msg.sender, address(this), package.total.mul(price).mul(2)) == true);
            ...
            decimal = 10 ** uint256(token.decimals());
            uint256 price = getTokenPrice(contractAddress);
            assert(token.transferFrom(msg.sender, address(this), package.total.mul(price)) == true);
...
//        return 10 ** 6;
	...
	function setOpenStar(bool _openStar) onlyAdmin public returns(uint) {
        openStar = _openStar;
        return SUCCESS;
    }
	
	function getOpenStar() onlyAdmin public returns(bool) {
        return openStar;
    }
    ...
    // usdt和币的收款地址
    address internal receiver_address = address(0x419aaff826a7b85910800f99f900519d15451a0aa8);
