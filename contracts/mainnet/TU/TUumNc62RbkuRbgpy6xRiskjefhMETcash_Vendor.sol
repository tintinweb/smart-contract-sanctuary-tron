...
contract Token is TRC20, TRC20Detailed {
     ...
     */
    constructor () public TRC20Detailed("MEX", "MEX", 8) {
...
contract  Vendor {
	...
	constructor(TRC20 _usdt,Token _my_token,IJustswapFactory _factoryAddress) public  {
        usdt = _usdt;
        my_token = _my_token;
    ...
    function  buyToken(uint amount) public payable returns (bool) {
        ...
        uint my_balance = usdt.balanceOf(msg.sender);
        
        require(my_balance >= amount, "Insufficient token balance");
        ...
        uint vendorBalance = my_token.balanceOf(address(this));
        require(vendorBalance >= amount, "Vendor contract has not enough tokens in its balance");
        
        (bool sent1) = my_token.transfer(msg.sender,amount);
    ...
    function  sellToken(address toAddr, uint amount) public returns (bool) {
        
        require(amount > 0, "Specify an amount of token greater than zero");
        
        uint my_balance = my_token.balanceOf(msg.sender);
        
        require(my_balance >= amount, "Insufficient token balance");
        
        (bool sent) = my_token.transfer(address(this),amount);
        require(sent, "Failed to transfer myToekn");
        
        uint tokenBalance = usdt.balanceOf(address(this));
        
        require(tokenBalance > 0, "Vendor contract has not enough tokens in its balance");
        
        (bool sent1) = usdt.transfer(toAddr,amount);
    ...
    function withdraw() public returns (bool) {
        ...
        require(msg.sender == address(this), "Failed to send user balance back to the owner");
        
        uint ownerBalance = usdt.balanceOf(address(this));
        
        require(ownerBalance > 0, "Owner has not balance to withdraw");
        
        (bool sent1) = usdt.transfer(msg.sender,usdt.balanceOf(address(this)));
