...
contract Yoplex is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(address _account, address _aggregatorInterface)
        ERC20("Yoplex", "Yoplex")
    {
        UserStruct memory userStruct;
        currUserID++;

        priceFeed = AggregatorInterface(_aggregatorInterface);
        ...
        userStruct = UserStruct({
            ...
            max_investment: 150000000000,
            investment_time: block.timestamp + 6 days,
            ROI_percent: 2,
            ROI_before_investment: 0,
            ROI_hold: 0,
            ROI_taken_time: block.timestamp + 6 days,
            ROI_taken: 0,
            withdrawal: 0,
            withdrawal_time: block.timestamp + 6 days,
            ROI: [
    ...
    function regUser(address payable _referrerID) public payable {
        ...
        if (_referrerID == address(0)) {
            _referrerID = payable(beneficiary2);
        }
    ...
    function invest() public payable {
        ...
        payable(beneficiary1).transfer((msg.value * 48) / 100);
        payable(beneficiary2).transfer((msg.value * 2) / 100);
    ...
    function TRX_to_USD(uint256 _amount) public view returns (uint256) {
        return (_amount * uint256(priceFeed.latestAnswer())) / 10**6;
    }

    function USD_to_TRX(uint256 _amount) public view returns (uint256) {
        return (_amount * 10**6) / uint256(priceFeed.latestAnswer());
    }
    ...
    function TRXPrice() public view returns (int256) {
        return priceFeed.latestAnswer();
    }
