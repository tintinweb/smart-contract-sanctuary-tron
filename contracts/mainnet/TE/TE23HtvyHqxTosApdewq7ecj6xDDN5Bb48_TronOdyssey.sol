...
library Errors {
    ...
    string constant OPERATION_NOT_APPROVED =
        "Operations with token not approved";
    ...
    string constant MARKET_MINIMUM_PRICE =
        "Market price cannot be lowet then 'minimumMarketPrice'";
...
contract TronOdyssey is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    using EnumerableSet for EnumerableSet.UintSet;
    using Strings for uint256;
    // using EnumerableSetMarket for EnumerableSetMarket.MarketLotSet;

    string internal _name = "TRON ODYSSEY";
    string internal _symbol = "ODYS";

    string public baseURI = "https://cdn.tronodyssey.com/metadata/";

    uint256 internal totalMinted = 0;
    uint256 internal totalMigrated = 0;

    uint256 public constant MINTING_LIMIT = 999;
    uint256 public constant MAX_BATCH_MINT = 100;

    uint256 public mintingPrice = 300000000;
    uint256 public minimumMarketPrice = 900000000;

    uint256 internal ownerComissionNom = 5;
    uint256 internal constant comissionDenom = 100;

    //Service:
    bool internal isReentrancyLock = false;
    bool internal isMarketLocked = false;
    address internal contractOwner;
    address internal constant addressZero = address(0);
    ...
    uint256 internal currentGiveawayTokens = 0;
    uint256 internal constant maxGiveawayTokens = 10;
    ...
    /*************************************************************************** */
    //                             Mint:
    function mint(uint256 numAliens)
        external
        payable
        reentrancyGuard
        mintingEnabled
    {
        uint256 price = numAliens * mintingPrice;
        require(msg.value >= price, Errors.NOT_ENOUGH_TRX);
        require(numAliens <= MAX_BATCH_MINT, "Too many aliens");

        if (msg.value > price) {
            payable(msg.sender).transfer(msg.value - mintingPrice);
        }

        for (uint256 i = 0; i < numAliens; i++) {
            _mint(msg.sender);
        }
    }
        ...
        marketLock
    {
        ...
        uint256 ownerComission = (_price * ownerComissionNom) / comissionDenom;
        payable(contractOwner).transfer(ownerComission);
        payable(tokenToOwner[_tokenId]).transfer(_price - ownerComission);
        emit MarketTrade(
            _tokenId,
            tokenToOwner[_tokenId],
            _to,
            msg.sender,
            _price
        );
    ...
    function giveawayToken(address _to) external onlyOwner {
        require(
            currentGiveawayTokens < maxGiveawayTokens,
            "No more giveaway tokens"
        );
        currentGiveawayTokens++;
        _mint(_to);
    }
    ...
    {
        require(
            (totalMinted + totalMigrated) < MINTING_LIMIT,
            Errors.ALL_MINTED
        );
    ...
    function _addToken(address _to, uint256 _tokenId)
        private
        notZeroAddress(_to)
    {
        ...
        notZeroAddress(_to)
    {
        ...
        if (isContract(_to)) {
            bytes4 retval = ITRC721TokenReceiver(_to).onTRC721Received(
                msg.sender,
                _from,
                _tokenId,
                _data
    ...
    function _setMinimumMarketPrice(uint256 newPrice)
        external
        onlyOwner
        marketLock
    {
        require(
            newPrice <= minimumMarketPrice,
            "The minimum price cannot be increased"
        );
        require(newPrice > 0, "Price must be greater than zero");
        minimumMarketPrice = newPrice;
    }

    function _setComission(uint256 ownerComission) external onlyOwner {
        if (ownerComission > 0) ownerComissionNom = ownerComission;
    }
    ...
    // Token migration
    function _migrateToken(uint256 _tokenId, address _to)
        external
        isMigration
        onlyOwner
        notZeroAddress(_to)
    ...
    // Token migration
    function _migrateTokens(uint256[] memory _tokenId, address[] memory _to)
        external
        isMigration
        onlyOwner
    {
    ...
    function setContractOwner(address _contractOwner) external onlyOwner {
    ...
    function getOwnerComission()
        external
        view
        returns (uint256 nominator, uint256 denominator)
    {
        return (ownerComissionNom, comissionDenom);
    }
