...
interface Imetedao {
  function bindParent(address account) external;
}
...
contract YHstake is LPTokenWrapper, Ownable {
    IERC20 public token;
    ...
    function setbindParent(address parent, address user) external {
      require(parent != address(0), "ERROR external parent");
    ...
    function bindParentOnlyOwner(address parent, address user) external onlyOwner{
      require(parent != address(0), "ERROR external parent");
      _parents[user] = parent;
      _mychilders[parent].push(user);
    }
