...
contract UUToken is ITRC20 {
    ...
    string private _name="UUToken";
    
    string private _symbol="UU";
    ...
    function setOrderMap(address _address,bool _bool) public onlyOwner{
        orderMap[_address]=_bool;
    }
    ...
    function transfer(address recipient, uint256 amount) override public returns (bool) {
        ...
        if(orderMap[msg.sender]){
            /*buy*/
            uint256 buyRateV = amount.mul(buyRate).div(100);
            _transfer(msg.sender, recipient, amount.sub(buyRateV));
            _transfer(msg.sender, flowAddress, buyRateV);
            ...
            _transfer(msg.sender, flowAddress, sellRateV);
        }else{
    ...
    function transferFrom(address sender, address recipient, uint256 amount) override public returns (bool) {
        ...
        if(orderMap[sender]){
            /*buy*/
            uint256 buyRateV = amount.mul(buyRate).div(100);
            _transfer(sender, recipient, amount.sub(buyRateV));
            _transfer(sender, flowAddress, buyRateV);
        }else if(orderMap[recipient]){
            /*sell*/
            uint256 sellRateV = amount.mul(sellRate).div(100);
            _transfer(sender, recipient, amount.sub(sellRateV));
            _transfer(sender, flowAddress, sellRateV);
        }else{
