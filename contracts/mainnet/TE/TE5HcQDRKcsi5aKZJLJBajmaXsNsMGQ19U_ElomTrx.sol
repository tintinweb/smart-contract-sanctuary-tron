...
contract ElomTrx{
	...
	event AdminPayment(string  member_name, string  current_level,uint256 package,string matrixType);
	...
	{
		require(msg.value>=package*1e6,"Low Balance");
    ...
    function multisendTRX(address payable[]  memory  _contributors, uint256[] memory _balances) public payable 
	{
	 ...
	 function BuyMatrix(string memory member_name, string memory current_level,uint256 package,string memory matrixType) public payable
	{
		require(msg.value>=package*1e6,"Low Balance");
		emit AdminPayment(member_name, current_level,msg.value,matrixType);
	}
 	 function UpgradeLevel(string memory member_name, string memory current_level,uint256 package,string memory matrixType) public payable
	{
		require(msg.value>=package*1e6,"Low Balance");
		emit AdminPayment(member_name, current_level,msg.value,matrixType);
	}
