...
contract eMavro is ERC20 {
    constructor() ERC20("eMavro", "EMV") {
...
contract ExchangeVendor is Ownable {
    ...
    constructor(address tokenAddress) {
        emvToken = eMavro(tokenAddress);
    }

    function changeTokenRate(uint256 _tokensPerBNB) public onlyOwner {
        require(
            _tokensPerBNB > 0,
            "Specify an rate of token greater than zero"
        );
    ...
    function buyTokens() public payable returns (uint256 tokenAmount) {
        ...
        uint256 vendorBalance = emvToken.balanceOf(address(this));
        require(
            vendorBalance >= amountToBuy,
            "Vendor contract has not enough tokens to perform transaction"
        );

        bool sent = emvToken.transfer(msg.sender, amountToBuy);
    ...
    function sellTokens(uint256 tokenAmountToSell) public {
        ...
        uint256 userBalance = emvToken.balanceOf(msg.sender);
        ...
        bool sent = emvToken.transferFrom(
            msg.sender,
            address(this),
            tokenAmountToSell
        );
