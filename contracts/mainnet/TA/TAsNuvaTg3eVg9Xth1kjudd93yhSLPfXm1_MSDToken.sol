...
contract MSDToken is Context, IERC20, Ownable {
    ...
    string private _name = 'Metaverse Pawn System Equity';
    ...
    uint256 private _previousPartnerFee = _partnerFee;
    ...
    uint256 private _partnerFeeTotal;
    uint256 private _marketinFeeTotal;
    ...
    function totalPartnerFee() public view returns (uint256) {
        return _partnerFeeTotal;
    }
    
    function totalMarketinFee() public view returns (uint256) {
        return _marketinFeeTotal;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 tTransferAmount, uint256 tFund, uint256 tPartner, uint256 tMarketing, uint256 tLiquidity) = _getValues(tAmount);
        ...
        if(
            ...
            _balances[_partnerAddress] = _balances[_partnerAddress].add(tPartner);
            _partnerFeeTotal = _partnerFeeTotal.add(tPartner);
            _balances[_marketingAddress] = _balances[_marketingAddress].add(tMarketing);
            _marketinFeeTotal = _marketinFeeTotal.add(tMarketing);
            ...
            emit Transfer(sender, _partnerAddress, tPartner);
    ...
    function calculatePartnerFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_partnerFee).div(
            10 ** 2
        );
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFund, uint256 tPartner, uint256 tMarketing, uint256 tLiquidity) = _getTValues(tAmount);

        return (tTransferAmount, tFund,  tPartner, tMarketing, tLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256,uint256, uint256, uint256) {
        ...
        uint256 tPartner = calculatePartnerFee(tAmount);
        uint256 tMarketing = calculateMarketingFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFund).sub(tPartner);
    ...
    function removeAllFee() private {
        if(_fundFee == 0 && _partnerFee == 0 && _marketingFee == 0 && _liquidityFee == 0) return;
        _previousFundFee = _fundFee;
        _previousPartnerFee = _partnerFee;
    ...
    function restoreAllFee() private {
        ...
        _partnerFee = _previousPartnerFee;
