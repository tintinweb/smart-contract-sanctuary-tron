...
contract TronPlus_COne is Ownable {
    ...
    address payable public PROMOTION_ADDRESS = address(0x41657DE9F82320D6E87B4DADD6A82FC4285055141C);
    uint256[4] public PROMOTION_PERCENTS = [20, 20, 20, 20]; 

    address payable public constant DEFAULT_REFERRER = address(0x41657DE9F82320D6E87B4DADD6A82FC4285055141C);
    ...
    event NewDeposit(uint256 depositId, address account, address referrer, uint8 depositType, uint256 amount);
    ...
    event TransferLeaderBonusReward(address indexed _to, uint256 indexed _amount, uint8 indexed _level);
    ...
    function makeDeposit(address payable ref, uint8 depositType) external payable {
        ...
        require(msg.value >= MINIMAL_DEPOSIT, "Not enought for mimimal deposit");
    ...
    function takeAwayDeposit(uint256 depositId) external returns (uint256) {
        ...
        require(deposit.freezeTime.add(PLANS_PERIODS[deposit.depositType]) <= block.timestamp, "Not allowed now");
    ...
    function distributeBonuses(uint256 _amount, address payable _player) private
    {
            ...
            for (uint8 j = 0; j < LEADER_BONUS_TRIGGERS.length; j++) {
                if (players[ref].leadTurnover >= LEADER_BONUS_TRIGGERS[j]) {
                    if (!players[ref].receivedBonuses[j] && address(this).balance >= LEADER_BONUS_REWARDS[j]) {
                        ...
                        emit TransferLeaderBonusReward(ref, LEADER_BONUS_REWARDS[j], i);
                    } else {
                        continue;
                    }
                } else {
    ...
    function getWithdrawnReferalFunds(address _player) public view returns (uint256) {
        return players[_player].referralReward;
    }

    function getWithdrawnLeaderFunds(address _player) public view returns (uint256) {
