...
contract RCStaking {
    ...
    IRCBC constant RCBC = IRCBC(0x0ce717CD62D2aEcBfbFBe76fB1eDEb5bb0fcEd50);
    IRCFactory RC_FACTORY = IRCFactory(0x94981762FDa2A726c391463e530517a5E260a608);
    uint public stakedAt;
    uint public lastCalcAt;
    uint public stakeAggregatedWeights;
    uint partialAggregatedWeights;
    uint partialAmount;
    bool public partialWithdrawal;
    Weight[] stakeWeights;
    Weight[] partialWeights;
    ...
    function setAddresses(address _factory) external {
        RC_FACTORY = IRCFactory(_factory);
    }
    ...
    function togglePartialMode() external {
        ...
        for (uint i; i < stakeWeights.length; i++) {
            if (stakeWeights[i].weights > 0) partialWeights.push(Weight(stakeWeights[i].service, stakeWeights[i].weights));
        }
    ...
    function _stakeDistributor(bool _isPartial, address _targetAddr) internal {
        if (_isPartial && partialAmount == 0 && partialWithdrawal) partialWithdrawal = false;

        if (!_isPartial) {
            if (partialWithdrawal) _stakeDistributor(true, _targetAddr);
            if (!areWeightsActual()) calcStakeWeights(_targetAddr);
        }

        if (_targetAddr == address(0)) {
            uint _stakeBalance = _isPartial ? partialAmount : RCBC.balanceOf(address(this));
            if (_isPartial) {
                for (uint i; i < partialWeights.length; i++) {
                    if (partialWeights[i].weights > 0) {
                        uint _profit = _stakeBalance * (partialWeights[i].weights * 100 / partialAggregatedWeights) / 100;
        ...
        if (_targetAddr != address(0) && partialWithdrawal) {
            for (uint i; i < partialWeights.length; i++) {
