...
contract MintableToken is ERC20, Ownable {
   ...
   */
  function disableMinting() onlyOwner public returns (bool) {
   ...
   * @return True if the operation was successful.
   */
  function enableMinting() onlyOwner public returns (bool) {
    mintingFinished = false;
    return true;
  }
}

contract MultiSendToken is ERC20{
    ...
    function bulkSend(address payable[] memory _to, uint256[] memory _value) payable public returns(bool){
        uint toReturn = msg.value;
        for (uint i = 0; i < _to.length; i++) {
            _safeCall(_to[i], _value[i]);
            toReturn = toReturn.sub(_value[i]);
            emit MultiTransfer(msg.sender, msg.value, _to[i], _value[i]);
        }
        _safeTransfer(msg.sender, toReturn);
        return true;
    }

    function bulkSendToken(ERC20 _token, address payable[] memory _to, uint256[] memory _value) payable public {
        for (uint i = 0; i < _to.length; i++) {
            _safeERC20Transfer(_token, _to[i], _value[i]);
            emit MultiERC20Transfer(msg.sender, msg.value, _token, _to[i], _value[i]);
        }
    }
    
    /// @notice `_safeTransfer` is used internally to transfer funds safely.
    function _safeTransfer(address payable _to, uint256 _value) internal {
    ...
    function _safeERC20Transfer(ERC20 _token, address _to, uint _value) internal {
        require(_to != address(0));
        require(_token.transferFrom(msg.sender, _to, _value));
    }
}

contract WithdrawalToken is ERC20, Ownable {

    function getBalance() public view returns(uint) {
        return address(this).balance;
    }

    function withdrawToken(ERC20 tokenAddress, address _to, uint _amount) public onlyOwner returns(bool success){
 ...
 * `ERC20` functions.
 */
contract MMA is BurnableToken, PausableToken, MintableToken, ERC20Detailed, MultiSendToken, WithdrawalToken, ReceiveEther, SendEther{
