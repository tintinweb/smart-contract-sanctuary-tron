 ...
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐
 *   │   Website: https://www.Tronicon.live    								 │
 * 	 │                                                                       │
 *	 │	 1% Daily ROI + 1.2% PERSONAL HOLD-BONUS 						     │
 *	 │                                                                       │
 ...
 *   [AFFILIATE PROGRAM]
 *
 *   11-level referral commission: 10% - 3% - 1% - 0.5% - 0.2% - 0.1% - 0.05% - 0.05% - 0.05% - 0.03% - 0.02% 
 *   
 */
...
contract Tronicon {
    ...
    uint[] public REFERRAL_PERCENTS = [1000, 300, 100, 50, 20, 10, 5, 5, 5, 3, 2];
    uint constant public MARKETING_FEE = 700; 
    uint constant public SYSTEM_FEE = 500;
    ...
    uint constant public CONTRACT_BALANCE_STEP = 500000000 trx;
    uint constant public LEADER_BONUS_STEP = 10000000  trx;
    ...
    function invest(address referrer) public payable {
        ...
        uint systemFee = msgValue.mul(SYSTEM_FEE).div(PERCENTS_DIVIDER);
		uint adminFee = msgValue.mul(ADMIN_FEE).div(PERCENTS_DIVIDER);
		

        marketingAddress.transfer(marketingFee);
        projectAddress.transfer(systemFee);
		adminAddress.transfer(adminFee);
		

        emit FeePayed(msg.sender, marketingFee.add(systemFee));
