...
contract PDGToken is ITRC20 {
    ...
    address public fundAddress=address(0x4f1652F3A84Aa364b01719cDa750d23E5c661317);
    ...
    function setFundAddress(address _flowAddress) public onlyOwner{
        fundAddress = _flowAddress;
    }
    ...
    function setSwapPairAddress(address _sunSwapPair) external onlyOwner {
     ...
     function setLPMarketFees(uint256 _lpFee,uint256 _marketFee) external onlyOwner {
        lpFee = _lpFee;
    ...
    function transfer(address recipient, uint256 amount) override public returns (bool) {
        ...
        }else if(orderMap[msg.sender]){
            /*buy*/
            uint256 sellRate = marketFee + lpFee;
            uint256 sellRateV = amount.mul(sellRate).div(100);
            _transfer(msg.sender, recipient, amount.sub(sellRateV));
            _transfer(msg.sender, sunSwapPair, amount.mul(lpFee).div(100));
            _transfer(msg.sender, fundAddress, amount.mul(marketFee).div(100));
    ...
    function transferFrom(address sender, address recipient, uint256 amount) override public returns (bool) {
        ...
        }else if(orderMap[sender]){
            /*buy*/
            uint256 sellRate = marketFee + lpFee;
            uint256 sellRateV = amount.mul(sellRate).div(100);
            _transfer(sender, recipient, amount.sub(sellRateV));
            _transfer(sender, sunSwapPair, amount.mul(lpFee).div(100));
            _transfer(sender, fundAddress, amount.mul(marketFee).div(100));
        }else if(orderMap[recipient]){
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            /*sell*/
            uint256 sellRate = marketFee + lpFee;
