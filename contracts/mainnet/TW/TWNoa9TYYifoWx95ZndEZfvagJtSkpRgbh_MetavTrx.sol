...
contract MetavTrx is Ownable{
    
 	using SafeMath for uint256;
    
    address payable private adminAccount_;
    
    event Registration(string  waddress,address investor,uint256 referrerId,uint256 package,string position);
    ...
    function UserRegister( string memory _user,uint256 _referrerCode,uint256 package,string memory position) public payable
    {  

        require(msg.value==package,"Invalid Amount");
	   payable(owner()).transfer(address(this).balance);
        emit Registration(_user,msg.sender,_referrerCode,msg.value,position);
    }
