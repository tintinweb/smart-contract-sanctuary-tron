...
//SourceUnit: BlockPurseBasePay.sol
...
abstract contract BlockPurseBasePay {
    ...
    event Order(string orderId, uint256 orderAmount,address settleToken,uint256 fee,address merchant, address payer, uint256 rate);
...
//SourceUnit: BlockPursePay.sol

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "./BlockPurseBasePay.sol";

contract BlockPursePay is BlockPurseBasePay {
    ...
    function pay(
        ...
        (uint256 totalFee, uint256 platformFee, uint256 proxyFee) = getPaymentFee(merchantInfo.rate, _orderAmount);

        if (merchantInfo.autoSettle) {

            _autoWithdraw(_merchant, merchantInfo.settleAccount, _currency, _orderAmount - totalFee);

        } else {

            merchantFunds[_merchant][_currency] += (_orderAmount - totalFee);

        }

        merchantFunds[merchantInfo.proxy][_currency] += proxyFee;
        tradeFeeOf[_currency] += totalFee;

        emit Order(_orderId, _orderAmount, _currency, totalFee, _merchant, msg.sender, merchantInfo.rate);
