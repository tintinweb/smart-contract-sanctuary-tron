...
contract GSTTokenContract is ST20 ('Global Success Team', 'GST') {
    ...
    uint256 public LiquidityPoolFee = 200 ;
    uint256 public BurningFee = 100;
    uint256 public HoldersFee = 500;
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        if(takeFee){
            uint256 charity_Fee = amount.mul(charityFee).div(10000);
            uint256 Admin_Fee = amount.mul(AdminFee).div(10000);
            uint256 LiquidityPool_Fee = amount.mul(LiquidityPoolFee).div(10000);
            uint256 Burning_Fee = amount.mul(BurningFee).div(10000);
            uint256 Holders_Fee = amount.mul(HoldersFee).div(10000);

            uint256 totalFees = charity_Fee.add(Admin_Fee).add(LiquidityPool_Fee).add(Burning_Fee).add(Holders_Fee);
            _transfer(_msgSender() , charityAddress , charity_Fee);
            _transfer(_msgSender() , adminAddress , Admin_Fee);
            _transfer(_msgSender() , LiquidityPoolAddress , LiquidityPool_Fee);
            _transfer(_msgSender() , BurningAddress , Burning_Fee);
            _transfer(_msgSender() , HoldersAddress , Holders_Fee);
    ...
    function transferFrom (address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if(takeFee){
            ...
            uint256 totalFees = charity_Fee.add(Admin_Fee).add(LiquidityPool_Fee).add(Burning_Fee).add(Holders_Fee);
            _transferFrom(sender , charityAddress , charity_Fee);
            _transferFrom(sender , adminAddress , Admin_Fee);
            _transferFrom(sender , LiquidityPoolAddress , LiquidityPool_Fee);
            _transferFrom(sender , BurningAddress , Burning_Fee);
            _transferFrom(sender , HoldersAddress , Holders_Fee);
    ...
    function setHoldersAddress(address _address) public onlyOwner{
        HoldersAddress = _address;
    }
