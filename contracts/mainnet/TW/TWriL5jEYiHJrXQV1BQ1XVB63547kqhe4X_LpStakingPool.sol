...
contract LpStakingPool is USDTWrapper,Ownable {
    ...
    function rewardPerToken() public view  returns (uint256) {
        ...
        return sAll.div(totalSupply());
    }

    function earned(address account) public view returns (uint256) {
        return   balanceOf(account).mul(rewardPerToken());
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        uint today= block.timestamp/(3600*24);
        uint lastday= getRewardTime[msg.sender]/(3600*24);
        require(today!=lastday, 'today has reward');
        ...
        if (reward > 0) {
            ...
            if ( p!= address(0)) {
                stakeOutToken.safeTransfer(p, reward * 18 / 100);
                p = referrerAddress[p];
                if (p != address(0)) {
                    stakeOutToken.safeTransfer(p, reward * 10 / 100);
                    p = referrerAddress[p];
                    if (p != address(0)) {
                        stakeOutToken.safeTransfer(p, reward * 5 / 100);
                    }
                }
            }
