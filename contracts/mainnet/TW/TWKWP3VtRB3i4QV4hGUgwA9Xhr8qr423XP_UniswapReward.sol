...
contract LPTokenWrapper is IPool,Governance {
    ...
    IERC20 public _TokenOne = IERC20(0xD90c02e3E01829120D8a0C85b60Abb1B7933486C); //todo
    ...
    function setproportion(uint _newproportion) public  onlyGovernance {
        //return _totalSupply;
        proportion = _newproportion;
    }
    ...
    function withdraw(uint256 amountone) public {
        ...
        _TokenOne.transfer( msg.sender, amountone);
    }
...
contract UniswapReward is LPTokenWrapper{
    ...
    IERC20 public _rewardToken = IERC20(0x7DBb48fDF748A11231133f464D131a0D9aec8795);    //todo
    
    
    address public inviteADDR;          //
    
    int128 private dayNums;
    uint256 public  DURATION = 30 days;       //todo
    uint256 public _initReward = 10000 * 1e18;
    
    uint public durationCounts = 0;
    ...
    constructor(address _newinviteaddr) public {
    ...
    function stake(uint256 amountone)
        public
        ...
        checkStart
    {
        ...
        super.stake(amountone);

        _lastStakedTime[msg.sender] = now;

    }

    function withdraw(uint256 amountone)
        public
        updateReward(msg.sender)
        checkHalve
        checkStart
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            if(leftReward>0){
                ...
                uint invitefee;
                invitefee = IPlayerBook(inviteADDR).settleReward(msg.sender,leftReward);
                leftReward = leftReward.sub(invitefee);
                
                _rewardToken.transfer(inviteADDR, invitefee );
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish) {
            durationCounts = durationCounts.add(1);
            if(durationCounts > 2)
            {
                _initReward = _initReward.div(10);
            }
