...
contract GU is TRC20, TRC20Detailed {
     ...
     */
    constructor (address _addressReceiver) public TRC20Detailed("Garden Universe", "GU", 18) {
        _mint(msg.sender, 100000000 * (10 ** uint256(decimals())));
         adminlist[msg.sender] = 1;
         addressReceiver = _addressReceiver;
         addressTreasury = _addressReceiver;
         addressBurn = _addressReceiver;

        // ISunswapV2Router02 _sunswapV2Router = ISunswapV2Router02(sunswapRouter);
        // sunswapV2Pair = ISunswapV2Factory(_sunswapV2Router.factory())
...
/*----------------------------------------------------------------
 ...
 -----------------------------------------------------------------*/
    function withdrawTokenForOwner(uint256 amount) public onlyAdmin {
        this.transfer(addressReceiver, amount);
    ...
    function withdrawBUSDForOwner(address token_address, uint256 amount)
        ...
        onlyAdmin
    {
        GUTK = GU(token_address);
        GUTK.transfer(addressReceiver, amount);
        emit WithDraw(amount);
    }
/*
===================================================
