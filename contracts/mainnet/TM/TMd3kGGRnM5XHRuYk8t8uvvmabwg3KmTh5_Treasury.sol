...
contract Treasury {
    ...
    ITreasury private preTreasury = ITreasury(0x41cb773e62d9f22784d8a016495001384d800d2648); // TUX
    IERC20 private ctt = IERC20(0x4175097f5a2773f77d03bdd0fad55eff48a56001a2);
    
    uint[] levelStep = [100000000000, 500000000000];
    uint[] selfCommision = [9, 11, 13];
    ...
    constructor () public {
        ...
        ctt.approve(address(0x413f879775d8d4de1b3d339e9fa71d34f99c01d02d), MAX_INT);
    }
    ...
    function withdrawal(address userAddress) public returns (bool) {
        ...
        for(uint j=0;j<txs.length;j++) { 
            ...
            if(block.timestamp - utx.timestamp >= preTreasury.blockTimeDuration()) { 
                ...
                require(totalWithdrawal <= ctt.balanceOf(address(this)), "Inventory shortage");
                emit AutoWithdrawal(userAddress, utx.hash, utx.amount, afterCommission); 
                txs[j] = txs[txs.length-1];
                txs.pop();
                ctt.transfer(userAddress, totalWithdrawal);
                ...
                for(uint k=1;k<=5;k++) { 
                    ...
                    if(acc > 0) { 
                        ...
                        require(bonus <= ctt.balanceOf(address(this)), "Inventory shortage");
                        emit ReferralBonus(userAddress, refOfuser, k, bonus); 
                        ctt.transfer(refOfuser, bonus); 
                        tmpAddr = refOfuser;
                    } else {
                        break;
                    }
    ...
    function checkUserLevelUp(address userAddress) private {
        User storage user = users[userAddress];
        uint sum = preTreasury.accumulationOf(userAddress); 
        sum += accumulation[userAddress] / 100;
        ...
        for(uint i=0;i<childList.length;i++) { 
            ...
            sum += accumulation[childAddress] / 100;
        }
