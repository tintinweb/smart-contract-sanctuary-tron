...
pragma solidity 0.4.25;
    ...
    contract FMDAO {
        ...
        mapping(address => uint) public lotteryNum;
        ...
        function getStaticStatus() external view returns(uint, uint, uint) {
            ...
            uint perAmount;
            if(LeftTimeNum > 0){
                perAmount = SafeMath.div(SafeMath.mul(playerMap[msg.sender].lastRechargeAmount, 3 * LeftTimeNum), 1000);
            }
        ...
        function withdrawImpl(address addr) internal {
            ...
            uint value = SafeMath.sub(playerMap[addr].wallet, number);
            uint leftPool;
            ...
            uint count = SafeMath.div(handlingFee, 10 * 10 ** 6);
            if(count > 0){
                lotteryNum[msg.sender] = SafeMath.add(lotteryNum[msg.sender], count);
            }
        ...
        function exchangeLottery(address addr,uint num) external {
            require(lotteryNum[msg.sender] >= num, "Insufficient lottery number");
            lotteryNum[msg.sender] = SafeMath.sub(lotteryNum[msg.sender], num);
            game.buyLotto(num, addr, 0);
        }
        ...
        function StaticPayment(address addr) internal {
            ...
            if(totalAmount >= totalBalance) {
                ...
                uint perAmount = SafeMath.div(SafeMath.mul(playerMap[addr].lastRechargeAmount,  3 * LeftTimeNum), 1000);
        ...
        function increasePerformance(uint usdtVal) internal {
            DappDatasets.Player storage player = playerMap[msg.sender];
            uint length = 0;
            while(player.superior && length < 30) {
        ...
        function exchange(uint num) external {
            require(num >= 1, "Redeem at least 1");
            uint usdtAmount = SafeMath.mul(num, 10 * 10 ** 6);
            tether.transferFrom(msg.sender, this, usdtAmount);
            game.buyLotto(num, msg.sender, 1);
        }
        ...
        function register(address addr, address superiorAddr) internal{
            ...
            if(playerMap[addr].wallet > 0){
                    playerMap[addr].isExist = true;
                    playerMap[addr].subordinates = temp;
                    if(superiorAddr == address(0x0) || playerMap[superiorAddr].isExist == false){
                        playerMap[addr].superior = false;
                        playerMap[addr].superiorAddr = address(0x0);
                    }else{
                        playerMap[addr].superior = true;
                        playerMap[addr].superiorAddr = superiorAddr;
                    }
            }else{ 
    ...
    contract FMDAOGame {
        ...
        function buyLotto(uint count, address addr, uint8 buyType) external;
