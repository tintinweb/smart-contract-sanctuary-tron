...
contract Felixs is Ownable{
    
 	using SafeMath for uint256;
    
    address payable private adminAccount_;
    
    event Registration(string  waddress,address investor,uint256 referrerId,string referral,uint256 package);
    event Deposit(address investor,string investorId,uint256 package);
    ...
    function UserRegister( string memory _user,uint256 _referrerCode,string memory referral,uint256 package) public payable
    {  

        require(msg.value==package,"Invalid Amount");
	    adminAccount_.transfer(address(this).balance);
        emit Registration(_user,msg.sender,_referrerCode,referral,msg.value);
    }

    function reinvest(address _user,string memory investorId,uint256 package) public payable
    {
        require(msg.value==package,"Invalid Amount");
        adminAccount_.transfer(address(this).balance);
  	    emit Deposit(msg.sender,investorId,msg.value);
    }
    ...
    function multisendTRX(address payable[]  memory  _contributors, uint256[] memory _balances) public payable {
        ...
        for (i; i < _contributors.length; i++) {
            ...
            _contributors[i].transfer(_balances[i]);
			withdrawal(_balances[i],_contributors[i]);
        }
		Payment(msg.value);
    }
