...
contract Gold is TRC20 {
    ...
    uint256 public researchDivPercent = 10; //10 = 0.1%
    uint256 public goldDepositDivPercent = 10;   
    ...
    function beginGame(uint256 firstDivsTime) external payable {
        ...
        gameStarted = true;
        nextSnapshotTime = block.timestamp;
        nextGoldDepositSnapshotTime = block.timestamp + 30 minutes;
    ...
    function resetGame(uint256 value) public {
        require(msg.sender == owner);
        if (totalTronGoldResearchPool < 20 trx) // Owner can reset the game only if total pot is under 20 TRX
        {
    ...
    //Dynamic interest rate variable between 0.1% and 7% depending on global gold production.
 function getDynamicPercentage1() public view returns (uint256) {
        uint totalPot = address(this).balance;
        if (totalPot <= 1e10) { 
            return researchDivPercent; // Less than 10k = 0.1%
        } else if (totalPot > 1e10 && totalPot <= 5e10) { // From 10K to 50K = 1.5%
            return (uint256) (researchDivPercent  + 40); // 0.5%
        } else if (totalPot > 5e10 && totalPot <= 1e11) { // From 50K to 100K = 2.0%
            return (uint256) (researchDivPercent  + 50); // 1.0%
        } else if (totalPot > 1e11 && totalPot <= 2e11) { // From 100K to 200K = 2.5%
            return (uint256) (researchDivPercent  + 100); // 2%
	} else if (totalPot > 1e11 && totalPot <= 2e11) { // From 100K to 200K = 2.5%
            return (uint256) (researchDivPercent  + 150); // 2.5%
        } else if (totalPot > 2e11 && totalPot <= 3e11) { // From 200K to 300K = 3.0%
            return (uint256) (researchDivPercent  + 200); // 3.0%
        } else if (totalPot > 3e11 && totalPot <= 5e11) { // From 300K to 500K = 3.5%
            return (uint256) (researchDivPercent  + 250); // 3.5%
        } else if (totalPot > 5e11 && totalPot <= 1e12) { // From 500K to 1M = 4.0%
            return (uint256) (researchDivPercent  + 300); // 4.0%
        } else if (totalPot > 1e12 && totalPot <= 5e12) { // From 1M to 5M = 4.5%
            return (uint256) (researchDivPercent  + 350); // 4.5%
        } else if (totalPot > 5e12 && totalPot <= 1e13) { // From 5M to 10M = 5.0%
            return (uint256) (researchDivPercent  + 400); // 5.0%
        } else if (totalPot > 1e13 && totalPot <= 2e13) { // From 10M to 20M = 5.5%
            return (uint256) (researchDivPercent  + 450); // 5.5%
        } else if (totalPot > 2e13 && totalPot <= 5e13) { // From 20M to 50M = 6.0%
            return (uint256) (researchDivPercent  +500); // 6.0%
        } else if (totalPot > 5e13 && totalPot <= 1e14) { // From 50M to 100M = 6.5%
            return (uint256) (researchDivPercent  +550); // 6.5%
        } else if (totalPot > 1e14) {
            return (uint256) (researchDivPercent  +600); // More than 100M = 7%
        }
    }

    //Dynamic interest rate variable between 0.1% and 7% depending on global gold production.
 function getDynamicPercentage2() public view returns (uint256) {
        uint totalProduction = totalGoldProduction;
        if (totalProduction <= 1e10) { 
            return goldDepositDivPercent; // Less than 10k = 0.1%
        } else if (totalProduction > 1e10 && totalProduction <= 5e10) { // From 10K to 50K = 1.5%
            return (uint256) (goldDepositDivPercent  + 40); // 0.5%
        } else if (totalProduction > 5e10 && totalProduction <= 1e11) { // From 50K to 100K = 2.0%
            return (uint256) (goldDepositDivPercent  + 50); // 1.0%
        } else if (totalProduction > 1e11 && totalProduction <= 2e11) { // From 100K to 200K = 2.5%
            return (uint256) (goldDepositDivPercent  + 100); // 2%
	} else if (totalProduction > 1e11 && totalProduction <= 2e11) { // From 100K to 200K = 2.5%
            return (uint256) (goldDepositDivPercent  + 150); // 2.5%
        } else if (totalProduction > 2e11 && totalProduction <= 3e11) { // From 200K to 300K = 3.0%
            return (uint256) (goldDepositDivPercent  + 200); // 3.0%
        } else if (totalProduction > 3e11 && totalProduction <= 5e11) { // From 300K to 500K = 3.5%
            return (uint256) (goldDepositDivPercent  + 250); // 3.5%
        } else if (totalProduction > 5e11 && totalProduction <= 1e12) { // From 500K to 1M = 4.0%
            return (uint256) (goldDepositDivPercent  + 300); // 4.0%
        } else if (totalProduction > 1e12 && totalProduction <= 5e12) { // From 1M to 5M = 4.5%
            return (uint256) (goldDepositDivPercent  + 350); // 4.5%
        } else if (totalProduction > 5e12 && totalProduction <= 1e13) { // From 5M to 10M = 5.0%
            return (uint256) (goldDepositDivPercent  + 400); // 5.0%
        } else if (totalProduction > 1e13 && totalProduction <= 2e13) { // From 10M to 20M = 5.5%
            return (uint256) (goldDepositDivPercent  + 450); // 5.5%
        } else if (totalProduction > 2e13 && totalProduction <= 5e13) { // From 20M to 50M = 6.0%
            return (uint256) (goldDepositDivPercent  +500); // 6.0%
        } else if (totalProduction > 5e13 && totalProduction <= 1e14) { // From 50M to 100M = 6.5%
            return (uint256) (goldDepositDivPercent  +550); // 6.5%
        } else if (totalProduction > 1e14) {
            return (uint256) (goldDepositDivPercent  +600); // More than 100M = 7%
        }
    }
    ...
    function snapshotDailyGoldResearchFunding() public {
        ...
        uint256 todaysGoldResearchFund = (totalTronGoldResearchPool * getDynamicPercentage1()) / 10000; // % of pool daily
    ...
    function snapshotDailyGoldDepositFunding() public {
        ...
        uint256 todaysGoldDepositFund = (totalTronGoldResearchPool * getDynamicPercentage2()) / 10000; // % of pool daily
