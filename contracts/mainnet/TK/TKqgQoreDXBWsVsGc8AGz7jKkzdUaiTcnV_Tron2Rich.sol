...
contract Tron2Rich {
	...
	uint256 constant public TEAM_FEE = 10;
	...
	constructor(address payable marketingAddr, address payable projectAddr, address payable reserveAddr, address payable teamAddr) public {
		require(!isContract(marketingAddr) && !isContract(projectAddr) && !isContract(reserveAddr) && !isContract(teamAddr));
	...
	function invest(address referrer) public payable {
		...
		teamAddress.transfer(msg.value.mul(TEAM_FEE).div(PERCENTS_DIVIDER));
		emit FeePayed(msg.sender, msg.value.mul(MARKETING_FEE.add(PROJECT_FEE).add(RESERVE_FEE).add(TEAM_FEE)).div(PERCENTS_DIVIDER));
