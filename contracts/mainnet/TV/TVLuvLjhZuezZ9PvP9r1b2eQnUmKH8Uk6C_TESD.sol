...
//
abstract contract WrappedToken is Ownable, TRC10Wrapper {
    /**
    ...
    constructor(
        ...
        uint8 precision,
        trcToken tokenId
    ) TRC20(name, symbol) TRC10Wrapper(tokenId) {
        _decimals = precision;
    }
...
//
/**
 * @dev Contract for wrapping TRC10 version of REV into TRC20 version
 */
contract TESD is WrappedToken {
    // 100,000,000 TESD for wrapping and 100,000,000 more additionally
    uint256 constant ADDITIONAL_SUPPLY = 100 * 10**6;

    constructor(address additionalSupplyReceiver)
        WrappedToken("TESD-Token", "TESD", 0, 1002686)
    {
        _mint(additionalSupplyReceiver, ADDITIONAL_SUPPLY);
    }
}