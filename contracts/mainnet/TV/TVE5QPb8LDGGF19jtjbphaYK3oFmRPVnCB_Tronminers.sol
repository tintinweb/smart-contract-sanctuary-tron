...
contract Tronminers {
    ...
    event TMTWithdraw(address withdrawer, uint256 amount);
    ...
    function contractInfo() public view returns(uint256 trxx, uint256 totalDeposits, uint256 totalPayouts, uint256 totalInvestors, uint256 totalHolding, uint256 balance){
        trxx = address(this).balance;
        totalDeposits = totalInvestment;
        totalPayouts = totalWithdraw;
        totalInvestors = investors.length;
        totalHolding = totalHoldings;
        balance = contractBalance;
        return(trxx,totalDeposits,totalPayouts,totalInvestors,totalHolding,balance);
    }
    ...
    function deposit(address _referer) public payable{
        require(msg.value>=100,"Minimum 100 TRX allowed to invest");
        ...
        _distributeSFI(msg.sender,msg.value.mul(15).div(100));
    ...
    function _setReferral(address _addr, address _referral, uint256 _amount) private {
        ...
        if(users[_addr].referral == address(0)) {
            ...
            for(uint8 i = 0; i < referral_bonuses.length; i++) {
                ...
                else if(i>0 && users[_referral].referrals_per_level[i]>=1000e6){
                    users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
                }
    ...
    function _setReReferral(address _referral, uint256 _amount) private {
        for(uint8 i = 0; i < referral_bonuses.length; i++) {
            ...
            else if(i>0 && users[_referral].referrals_per_level[i]>=1000e18){
                users[_referral].levelIncome[i]+=_amount.mul(referral_bonuses[i].div(100)).div(100);
            }
            _referral = users[_referral].referral;
            if(_referral == address(0)) break;
    ...
    function _distributeSFI(address depositor, uint256 _sfi) internal{
        ...
        for(uint256 i = 0; i < investors.length; i++){
			...
			if(user.totalBusiness>=100000e6){
    ...
    function tMTWithdraw() public{
        ...
        emit TMTWithdraw(msg.sender,withdrawable);
    }
    ...
    function sellTMT(address payable buyer, uint _amount) external onlyInitiator{
