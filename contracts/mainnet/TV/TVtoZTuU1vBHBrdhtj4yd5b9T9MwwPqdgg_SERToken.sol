 ...
 */
contract SERToken is IERC20, Ownable {
    ...
    uint256 private _transferFeeRate = 30;
    ...
    constructor () public {
        ...
        _symbol = "SER";
        _decimals = 6;
        _mint(_msgSender(), 9800000 * 10**(uint256(_decimals)));
        _minTotalSupply = 98000 * 10**(uint256(_decimals));
    }
    ...
    function _checkAvailableTransferAndFee(address sender, address recipient, uint256 amount) private view returns (uint256 fee, uint256 rev) {
        // sender 不在黑名单，recipient 不在白名单情况下控制，配置 swap 在_fromBlackList，pool 在白名单，则 pool 仍然可以正常买卖，其他人不能买， swap在_toBlackList里面， 除了白名单都不能卖
