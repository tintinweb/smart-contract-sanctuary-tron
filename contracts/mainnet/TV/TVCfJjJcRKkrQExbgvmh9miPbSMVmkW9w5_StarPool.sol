...
contract StarPool is StarStorage {
    ...
    // 获取产品期数
    function GetActivityList() public view returns (uint status, uint[] memory idReturn, uint[] memory amountReturn, uint[] memory buyAmountReturn, bool[] memory openStatus) {
		idReturn = new uint[](1);
		amountReturn = new uint[](1);
		buyAmountReturn = new uint[](1);
		openStatus = new bool[](1);
		status = SUCCESS;
        for (uint i = 0; i < 1; i++) {
    ...
    function GetPackageList() public view returns (uint status, uint[] memory idReturn, int[] memory packageReturn, uint[] memory packageTotalReturn) {
        idReturn = new uint[](7);
        packageReturn = new int[](7);
        packageTotalReturn = new uint[](7);
        for (uint i = 0; i < 7; i++) {
    ...
    function buyPackage(uint index, address contractAddress, uint packageType) public payable returns (uint) {
		...
		uint256 addCal = package.total * 2;
		if (packageType == 2) {
            ...
            user.calp = user.calp.add(addCal);
            totalCalp = totalCalp.add(addCal);
    ...
    // 用户购买列表
    function GetUserPackageList(uint page, uint limit) public view userExist returns (address[] memory _ca , uint[6][] memory arList) {
        uint pl = userBuyPackageList[msg.sender].length;
		_ca = new address[](limit);
		arList = new uint[6][](limit);
    ...
    // 资产记录列表
    function GetAssetLog(uint page, uint limit) public view userExist returns (uint status, uint[] memory _amountReturn, uint[] memory _assetTypeReturn, uint[] memory _timeReturn) {
        AssetLog[] memory al = assetLogList[msg.sender];
		_amountReturn = new uint[](limit);
		_assetTypeReturn = new uint[](limit);
...
//        return 10 ** 6;
    ...
    **********************************************************************************************************************/
    constructor() public {
        ...
        packageList[3] = Package(3, 500, - 1);
        packageList[4] = Package(4, 1000, 300);
        packageList[5] = Package(5, 2000, 200);
        packageList[6] = Package(6, 5000, 100);
        packageList[7] = Package(7, 10000, 50);
