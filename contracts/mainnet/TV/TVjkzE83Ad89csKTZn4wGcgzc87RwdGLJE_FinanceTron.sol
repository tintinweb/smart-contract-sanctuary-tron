...
contract FinanceTron is Ownable{
    ...
    function getWhithdrableStat() public view returns (uint256 profit, uint256 profitOnProfit){
        return getWhithdrableStat(msg.sender);
    }
    ...
    function _calculateInternalBalance(uint256 amount, address addr) private returns (uint256 payableAmount){
        ...
        uint256 insuredMoney = investors[addr].totalInvestedMoney.mul(1000 - ADMIN_PERCENT).div(1000).mul(INSURED_PERCENT).div(100);
        if(addr == mainWallet)
            insuredMoney = amount;
    ...
    function getOveralStat() public view onlyOwner
    returns(uint256 totalReferrals,uint256 totalReferralEarned,uint256 totalDepositedMoney,uint256 totalWithdrawnMoney,uint256 normalBalanceValue,uint256 povertyBalanceValue, uint256 capacitorTotalMoney, bool Charging ,address capacitorAddress,uint256 currentBalance, uint256 PreviousBalance, uint256 TotalChargedAmount){
        bool isCH = isCharging();
        return (totalReferral, totalReferralEarn ,totalDeposit ,totalWithdrawn  ,normalBalance ,povertyBalance ,capacitorTotal ,isCH ,_capacitor, address(this).balance, previousBalance, totalChargedAmount);
    }
