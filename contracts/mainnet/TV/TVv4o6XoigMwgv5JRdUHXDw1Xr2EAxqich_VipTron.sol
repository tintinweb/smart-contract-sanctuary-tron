...
contract VipTron {
    ...
    address payable public fund_fee;
    address payable public admin_fee;
    address payable public ads_fee;
    ...
    constructor(address payable _owner) {
        ...
        admin_fee = payable(0x4a697899B60ba958B70eE40cD3C52B9d92bfE886);
        ads_fee = payable(0x6c64565Bb9E2b80d483d31b21eb093bdf11CA5D6);
        fund_fee = payable(0xF8f9bF3E7eAF6ba2F400739a59a5B58e2921f2aE);
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        fund_fee.transfer((_amount * 3) / 100);
        ads_fee.transfer((_amount * 5) / 100);
    ...
    */
    ...
    function poolTopInfo() view external returns(address[4] memory addrs, uint256[4] memory deps, uint256[4] memory incomes) {
        for(uint8 i = 0; i < pool_bonuses.length; i++) {
            if(pool_top[i] == address(0)) break;

            addrs[i] = pool_top[i];
            deps[i] = pool_users_refs_deposits_sum[pool_cycle][pool_top[i]];
            incomes[i] = users[pool_top[i]].total_payouts;
        }
    }
}