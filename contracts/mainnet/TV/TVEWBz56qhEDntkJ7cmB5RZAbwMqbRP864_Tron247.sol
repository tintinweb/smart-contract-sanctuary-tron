...
contract Tron247 {
    ...
    uint256 private minuteRate = 810185;  //DAILY 7%
    ...
    function deposit(address _affAddr) public payable {
        ...
        uint256 business = players[_affAddr].td_business;
        uint256 livng_time = now.sub(players[_affAddr].j_time);
        if (livng_time <= 604800) 
        {
            ...
            if (business >= 500000000000) {
                if (!Rewards[_affAddr].r2) {
                    player.reward_earned = player.reward_earned.add(
                        20000000000
                    );
                    _affAddr.transfer(10000000000);
        ...
        uint256 feed1earn =
            depositAmount.mul(devCommission).mul(8).div(commissionDivisor);
        uint256 feed2earn =
            depositAmount.mul(devCommission).mul(2).div(commissionDivisor);
    ...
    function collect(address _addr) internal {
        ...
        if (secPassed >  2073600  && player.time > 0) {
            secPassed = 2073600; // 24 days
