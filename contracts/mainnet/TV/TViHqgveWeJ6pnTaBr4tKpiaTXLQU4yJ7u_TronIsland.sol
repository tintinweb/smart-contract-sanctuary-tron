...
contract TronIsland is Pausable{
    ...
    uint256 constant public LOTTERY_PERCENT = 50;
    ...
    function register(address referrer) public {
        require(users[msg.sender].parent == address(0), "Inviter address was set in the TronIsland network!");
        require(users[referrer].parent != address(0) || referrer == owner(), "Inviter address does not exist in the TronIsland network!");
    ...
    function reinvest(uint256 amount, uint256 boxId, string memory predict) public whenNotPaused {
        ...
        address addr = msg.sender;
        require(users[addr].parent != address(0), "the address does not exist in the TronIsland network!");
