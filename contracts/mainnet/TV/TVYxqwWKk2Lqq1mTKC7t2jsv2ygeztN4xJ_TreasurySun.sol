//SourceUnit: CTokenInterfaces.sol
...
//SourceUnit: ExtendedSafeCastLib.sol
...
interface IMembership {
    function getUser(uint256 _index) external view returns (address);

    function getSponsor(address _user) external view returns (address);

    function getTotalMember() external view returns (uint256);

    function getParentTree(address _child, uint256 _deep)
        external
        view
    ...
    function isParent(address _parent, address _child)
        external
        view
        returns (bool);
}
...
interface ITreasury {
    function getInterest(address[] memory rwToken)
        external
        returns (
            uint256[] memory rwAmount,
            uint256 pre,
            uint256 current
        );

    function getLastCollect() external view returns (uint256);

    function isTreasury() external pure returns (bool);
}


//SourceUnit: IsuperGAUGE.sol

// SPDX-License-Identifier: MIT
pragma solidity 0.8.6;

interface IsuperGAUGE {
    ...
    function claimable_reward_for(address _wallet)
        external
        view
        returns (uint256);
}
...
//SourceUnit: OverflowSafeComparatorLib.sol
...
//SourceUnit: TreasurySun.sol

// SPDX-License-Identifier: MIT

pragma solidity 0.8.6;

import "./IsuperGAUGE.sol";
import "./IMinter.sol";
...
contract TreasurySun is ERC20Permit, ITreasury {
    using ExtendedSafeCastLib for uint256;

    bytes32 private SUPERGAUGEBYTES32 =
        0x00000000000000000000004140339f0c5608abd31b275bf95334eb3595a540f4;

    uint256 private constant MAXINT = 2**256 - 1;

    uint256 private _lastCollect;
    ...
    IERC20 public immutable _rwToken;

    IsuperGAUGE public immutable _superGauge;

    IMinter public immutable _minter;

    IMembership public immutable _member;

    event ClaimInterest(uint256[] rwAmount, uint256 pre, uint256 current);
     ...
     * @param account The recipient of the ticket power (may be the same as the user).
    ...
    constructor(
        ...
        address prizePool_,
        address membership_
    ) ERC20Permit("Treasury") ERC20(_name, _symbol) {
        require(decimals_ > 0, "Treasury/decimals-gt-zero");
        _decimals = decimals_;
        _superGauge = IsuperGAUGE(superGauge_);
        _lpToken = IERC20(IsuperGAUGE(superGauge_).lp_token());
        _rwToken = IERC20(IsuperGAUGE(superGauge_).rewarded_token());
        _minter = IMinter(IsuperGAUGE(superGauge_).minter());
        SUPERGAUGEBYTES32 = bytes32(
            uint256(uint160(superGauge_)) +
                94997606426508689683239514126558396277635615293440
        );
        prizePool = prizePool_;
        owner = msg.sender;
        _lastCollect = block.timestamp;
        _member = IMembership(membership_);
    }
    ...
    function getLastCollect() external view virtual override returns (uint256) {
        return _lastCollect;
    }

    function isTreasury() external pure virtual override returns (bool) {
        return true;
    }

    function changeMinDeposit(uint256 _newMinDeposit) public {
        require(msg.sender == owner, "Treasury/only-owner");
        minDeposit = _newMinDeposit;
    }

    function changePrizePool(address _newPrizePool) public {
        require(msg.sender == owner, "Treasury/only-owner");
        require(
            _newPrizePool != address(0x0),
            "Treasury/prize-pool-zero-address"
        );
        prizePool = _newPrizePool;
    }

    function approveToken() external {
        _lpToken.approve(address(_superGauge), MAXINT);
    }

    function mint(uint256 _amount, address _sponsor) external {
        require(_amount >= minDeposit, "Treasury/min-deposit");
        _member.addMember(msg.sender, _sponsor);
        _lpToken.transferFrom(msg.sender, address(this), _amount);
        _superGauge.deposit(_amount);
        _mint(msg.sender, _amount);
    }

    function burn(uint256 _amount) external {
        _superGauge.withdraw(_amount);
    ...
    {
        require(msg.sender == prizePool, "Treasury/only-call-by-prize-pool");
        if (_lastCollect >= block.timestamp)
            return (rwAmount, _lastCollect, _lastCollect);
        pre = _lastCollect;
        current = block.timestamp;
        _lastCollect = block.timestamp;
        _superGauge.claim_rewards();
        if (rwToken.length > 1) {
            _minter.claim(SUPERGAUGEBYTES32);
        }
        uint256[] memory _amount = new uint256[](rwToken.length);
        for (uint256 i = 0; i < rwToken.length; i++) {
            require(
                rwToken[i] != address(_lpToken),
                "Treasury/not-get-lp-token"
            );
            uint256 amount = IERC20(rwToken[i]).balanceOf(address(this));
            IERC20(rwToken[i]).transfer(prizePool, amount);
            _amount[i] = amount;
        }
        emit ClaimInterest(_amount, pre, current);
        return (_amount, pre, current);
    }

    function getInterestAmount() external view returns (uint256) {
        return _getInterestAmount();
    }
    ...
    function _getInterestAmount() internal view returns (uint256) {
        return _superGauge.claimable_reward_for(address(this));
    }

    /* ============ TWAB External Functions ============ */
    ...
    function _beforeTokenTransfer(
        ...
        _transferTwab(_from, _to, _amount);
    }
...
//SourceUnit: draft-ERC20Permit.sol
...
//SourceUnit: draft-IERC20Permit.sol
