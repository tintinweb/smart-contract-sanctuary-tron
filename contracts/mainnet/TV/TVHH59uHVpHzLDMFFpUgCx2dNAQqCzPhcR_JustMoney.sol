...
interface HandlerOracle {
    ...
    function approveMigrateContractChange() external returns (bool);
...
abstract contract BridgeOracle is Ownable {
    ...
    address private _migrateContract;

    event BridgeHandlerSet(address indexed added);
    event MigrateContractSet(address indexed added);
    ...
    function isMigrateContract(address account) public view returns (bool) {
        return _migrateContract == account;
    }
    ...
    modifier onlyOracleAndBridge() {
        ...
        require(isBridgeHandler(_msgSender()) || isMigrateContract(_msgSender()) || address(_handlerOracle) == _msgSender(), Errors.NOT_ORACLE_OR_HANDLER);
        _;
    }
    ...
    function approveOracleToSetMigrateContract() public onlyOwner returns (bool) {
        ...
        return _handlerOracle.approveMigrateContractChange();
    }
    ...
    function setMigrateContract(address account) public onlyHandlerOracle {
        require(account != address(0), Errors.OWNABLE_NOT_ZERO_ADDRESS);
        require(!isMigrateContract(account), Errors.ADDRESS_IS_MIGRATE);

        emit MigrateContractSet(account);
        _migrateContract = account;
    }
...
library Errors {
    ...
    string constant ADDRESS_IS_MIGRATE = "Oracle: Address is already the Migrate Contract";
...
contract JustMoney is BridgeOracle, IERC20 {
    ...
    function _mint(address account, uint256 amount) internal mintingEnabled notZeroAddress(account) belowCap(amount) {
        ...
        } else if ( isMigrateContract(mintBy) ) {
            emit MigrateMint(mintBy, account, amount);
        } else {
    ...
    event MigrateMint(address indexed by, address indexed to, uint256 value);
