...
contract Mbox is Ownable{
    ...
    uint public fundFee = 5;
    address public zoologyAddress;
    uint public zoologyFee = 15;
    ...
    ///////////////////////////////VIEW///////////////////////////////////
    function getMarketInfo(uint index)external view returns(MarketInfo memory info) {

        Market storage market = markets[index];
        info.total = market.total;
        info.priceAmount = market.priceWorth;

        (uint[] memory typeList,uint[] memory levelList) = nft.getTypesAndLevels();
        info.supportType = typeList;
        info.supportLevel = levelList;
    ...
    function getMarketInfos()external view returns(MarketInfo[] memory infos) {

        uint len = markets.length;

        infos = new MarketInfo[](len);

        for( uint i = 0; i < len; i++ ){
            Market storage market = markets[i];
            infos[i].total = market.total;
            infos[i].priceAmount = _worthConvert(market.priceWorth);

            (uint[] memory typeList,uint[] memory levelList) = nft.getTypesAndLevels();
            infos[i].supportType = typeList;
            infos[i].supportLevel = levelList;
    ...
    function _getRandomLevel(uint level,uint seed)internal pure returns(uint){
        uint v = seed % 100;
        if( level == 1){
            if( v < 84 ) return 1;
            if( v < 91 ) return 2;
    ...
    function buy(uint index,bytes32 flag) external  {
        require( !_isContract(msg.sender),"not allow");

        require( index < markets.length,"error");

        Market storage market = markets[index];

        require(market.total > 0,"surplus lock");
        market.total -= 1;

        uint256 tokenAmount = _worthConvert(market.priceWorth);

        uint fund = tokenAmount * fundFee / 100;
        if( fund > 0 ){
            payToken.transferFrom(msg.sender, fundAddress, fund);
            tokenAmount -= fund;
        }

        uint zoology = tokenAmount * zoologyFee / 100;

        if( zoology > 0 ){
            payToken.transferFrom(msg.sender, zoologyAddress, zoology);
            tokenAmount -= zoology;
        }
        ...
        uint level = _getRandomLevel(index + 1,seed);

        uint id = nft.mint(msg.sender, uint8(types), uint8(level));

        resultMap[flag] = Result(address(nft),id);

        _sendFirstShareAward(msg.sender,tokenAmount);
    }

    function _sendFirstShareAward(address user,uint amount) internal {
        address parent = relation.getParent(user);
        if( parent != address(0) ){
            uint v = amount * shareConfig / 100;
            payToken.transfer(parent, v);
        }
    }
    ...
    function newMarket(
            uint[] memory amounts,
            uint[] memory prices)external onlyOwner{
        uint len = amounts.length;
        require( len == prices.length,"error");
        for( uint i = 0; i < len; i++ ){
            markets.push( Market(
                prices[i],amounts[i]
            ));
    ...
    function updateMarketPrice(uint index,uint price)external onlyOwner{
        Market storage ma = markets[index];
        ma.priceWorth = price;
    }


    function updateMarket(uint index,uint amount)external onlyOwner{
         Market storage market = markets[index];
         market.total += amount;
     }
    ...
    function setNtoken(address token)external onlyOwner{
        nft = NtokenStandard(token);
    }
