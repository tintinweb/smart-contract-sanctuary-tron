...
contract FTLPPool is LPTokenWrapper, Inviter {
    ...
    constructor(address _token, address _lp) {
        token = IERC20(_token);
        lpToken = IERC20(_lp);
        startTime = 1648180800;
        initReward = 18000 * 10**6;
    ...
    function earned(address account) public view returns (uint256 _ea, uint256 _value) {
        ...
        address inviter = inviterMap(account);
    ...
    (address[] memory _users, uint256[] memory _values) {
        ...
        if (page > 0) {
            ...
            address[] memory inviters = getInviter(_account);
