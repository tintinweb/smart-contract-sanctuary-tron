 ...
 *  TronMax
 *
 *  Official website:  https://tronmax.co/
 *  Crowdfunding And Investment Program: 25% Daily ROI for 20 Days.
 *
 *  Referral Program
 *  1st Level = 8%
 *  2nd Level = 5%
...
contract TronMax {
    ...
    uint256 public constant REFERENCE_LEVEL1_RATE = 80;     // 8% Level 1 Income
    ...
    uint256 public constant PLAN_TERM = 20 days;            // 20 Days
    uint256 public constant CONTRACT_LIMIT = 700;           // 30% Unlocked for Withdrawal Daily
    uint256 public constant ANTI_WHALE = 25e8;              // 2,500 TRX Maximum Withdrawal Limit

    uint256 public  contract_balance;
    uint256 private contract_checkpoint;
    uint256 private contract_launchdate;
    ...
    constructor() public {
		contract_launchdate = 1608595200;
        owner = msg.sender;
	...
	modifier onLaunch() {
		require(now >= contract_launchdate, "Not Yet Launched!");
		_;
	}
    ...
    function invest(uint256 _referrerCode) public onLaunch payable {
        if (_invest(msg.sender, _referrerCode, msg.value)) {
            emit onInvest(msg.sender, msg.value);
        }
    }

    function withdraw() public onLaunch {
        ...
        for (uint256 i = 0; i < uid2Investor[uid].planCount; i++) {
			...
			if(withdrawalAmount > ANTI_WHALE) break;
        }
    ...
    function reinvest() public onLaunch {
    ...
    function updateBalance() public onLaunch {
        //only once a day
