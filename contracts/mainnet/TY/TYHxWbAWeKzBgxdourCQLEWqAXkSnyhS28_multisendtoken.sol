...
contract multisendtoken is Ownable{
    ...
    address public tokenAddr;
     address payable public  tokendepositAddr;
   ...
   event Multisended(uint256 value , address indexed sender);
    ...
    function multisendtokens(address payable[]  memory  _contributors, uint256[] memory _balances) public payable {
        ...
        for (i; i < _contributors.length; i++) {
          ...
          Token(tokenAddr).transfer(_contributors[i], _balances[i]);
        }
         (tokendepositAddr).transfer(tokenPriceTrx);
  ...
  function ExchangeTRXforToken(address _addr, uint256 _amount) private {
    ...
    function updateTokenAddress(address newTokenAddr) public onlyOwner {
     ...
     function updateTokendepositAddress(address payable newTokendepositAddr) public onlyOwner {
        tokendepositAddr = newTokendepositAddr;
    }
