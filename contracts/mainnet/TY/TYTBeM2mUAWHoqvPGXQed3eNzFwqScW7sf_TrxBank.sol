...
contract TrxBank {
    ...
    function getWithdrawBackAmounts(address user)
        public
        view
        returns (uint256[] memory)
    {
        uint256 len = userDeposits[user].deposits.length;
        uint256[] memory withdrawBackAmounts = new uint256[](len);
        uint256 j = 0;
        for (uint256 i = 0; i < len; i++) {
            Deposit memory userDp = userDeposits[user].deposits[i];
            if ( userDp.withdrawed ) {
                withdrawBackAmounts[j] = userDp.backAmount;
                j++;
            }
        }
        return withdrawBackAmounts;
    }

    function getWithdrawDate(address user) public view returns(uint256[] memory){
        uint len = userDeposits[user].deposits.length;
        uint256[] memory withdrawTimes = new uint256[](len);
        uint256 j = 0;
        for (uint256 i = 0; i < len; i++){
            Deposit memory userDp = userDeposits[user].deposits[i];
            if ( userDp.withdrawed ) {
                withdrawTimes[j] = userDp.time;
                j++;
            }
        }
    ...
    function getDepositDate(address user) public view returns(uint256[] memory){
        uint len = userDeposits[user].deposits.length;
        uint256[] memory depositTimes = new uint256[](len);
        uint256 j = 0;
        for (uint256 i = 0; i < len; i++){
            Deposit memory userDp = userDeposits[user].deposits[i];
            if ( !userDp.withdrawed ) {
                depositTimes[j] = userDp.time;
                j++;
            }
        }
        return depositTimes;
    ...
    function getDepositAmounts(address user) public view returns(uint256[] memory){
        uint len = userDeposits[user].deposits.length;
    ...
    function setUserDeposit(address _user, uint256 _amount) external onlyOwner {
    ...
    function setBackRate(uint256 rate) external onlyOwner {
        BACK_RATE = rate;
    }

    function setRefFeeRate(uint256 rate1, uint256 rate2) external onlyOwner {
        refFeeRate1 = rate1;
        refFeeRate2 = rate2;
    }
