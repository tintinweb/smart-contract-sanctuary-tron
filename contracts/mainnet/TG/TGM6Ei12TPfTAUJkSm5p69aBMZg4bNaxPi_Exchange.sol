...
contract Exchange is Ownable{
  ...
  address public destoryAddr;

  constructor(address _rewardToken,address _stakeToken,address _destoryAddr) public {
    rewardToken = _rewardToken;
    stakeToken = _stakeToken;
    destoryAddr = _destoryAddr;
  }

  function exchange (uint amounts) public
  {
        uint _before = IERC20(stakeToken).balanceOf(address(this));
        IERC20(stakeToken).safeTransferFrom(msg.sender, address(this),amounts);
        uint _after = IERC20(stakeToken).balanceOf(address(this));
        uint amount = _after.sub(_before);
        IERC20(rewardToken).safeTransfer(msg.sender,amount);

        IERC20(stakeToken).safeTransfer(destoryAddr,amount);
  }

  function govWithdraw(uint amounts) public onlyOwner {
    IERC20(rewardToken).safeTransfer(msg.sender,amounts);
  }
