...
contract TRexPunks is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    IERC721Enumerable private oldContract;
    ORACLE private _oracle;
    string public PROVENANCE = "";
    string public baseURI;
    string private baseExtension = ".json";
    uint256 public minSellPrice = 1000000000; // 1000 TRX
    uint256 public totalWithdrawForEvent = 0;
    uint256 public totalWithdrawForBuyBurnAdd = 0;
    ...
    uint256 private maxKeepForEvent = 53000000000; // 53000 TRX is kept to pay event rewards
    uint256 private keepFromEach = 212000000; // Keep 212 TRX from each NFT sale until it reach 53000 TRX (250 minted NFTs)
    uint256 private keepCounter = 0;
    uint256 private currentKeep = 0;
    ...
    mapping(uint256 => bool) private isClaimedEvent;
    mapping(uint256 => ForSale) private _forSale;
    mapping(address => ForSaleByUser) private _forSaleByUser;

    constructor(string memory _name, string memory _symbol, string memory _initBaseURI, address _oldContract, address _nftBuyFeeAddress, address _oracleAddress) ERC721(_name, _symbol) {
        _oracle = ORACLE(_oracleAddress);
        setBaseURI(_initBaseURI);
        oldContract = IERC721Enumerable(_oldContract);
    ...
    function mintNFT(uint256 _mintAmount) public payable {
        ...
        for(uint256 i = 0; i < _mintAmount; i++) {
            ...
            if (totalSupply() < maxSupply) {
                ...
                if(keepCounter < maxKeepForEvent) {
                    keepCounter += keepFromEach;
                    currentKeep += keepFromEach;
                }
            }
        }
    }
    ...
    function balanceToBuyBurnAdd() public view returns (uint256) {
        return address(this).balance - currentKeep;
    }
    ...
    /**
    * Function that mint the NFT's from the old contract upon creation.
    * It will only be used when contract is deployed
    */
    function _migrateOldNFTs() private onlyOwner {
        uint256 countOldNFT = oldContract.totalSupply();

        for(uint256 i = 0; i < countOldNFT; i++) {
            uint256 _tokenID = oldContract.tokenByIndex(i);
            address _ownerOf = oldContract.ownerOf(_tokenID);
            indices[_tokenID - 1] = _tokenID;
            _safeMint(_ownerOf, _tokenID);
            minted.push(_tokenID);
        }
    }
    ...
    */
    ...
    function withdrawForBuyBurnAdd() public onlyOwner {
        require(balanceToBuyBurnAdd() > 0, "No funds for buy, burn and add thingy");
        totalWithdrawForBuyBurnAdd += balanceToBuyBurnAdd();
        require(payable(msg.sender).send(balanceToBuyBurnAdd()));
    }

    function withdrawForEvent() public onlyOwner {
        require(currentKeep > 0, "Nothing to withdraw");

        if(totalSupply() >= 112 && isClaimedEvent[112] == false) {
            require(payable(msg.sender).send(5000000000));
            totalWithdrawForEvent += 5000000000;
            currentKeep -= 5000000000;
            isClaimedEvent[112] = true;
        } else if (totalSupply() >= 224 && isClaimedEvent[224] == false) {
            require(payable(msg.sender).send(9000000000));
            totalWithdrawForEvent += 9000000000;
            currentKeep -= 9000000000;
            isClaimedEvent[224] = true;
        } else if (totalSupply() >= 336 && isClaimedEvent[336] == false) {
            require(payable(msg.sender).send(13000000000));
            totalWithdrawForEvent += 13000000000;
            currentKeep -= 13000000000;
            isClaimedEvent[336] = true;
        } else if (totalSupply() >= 448 && isClaimedEvent[448] == false) {
            require(payable(msg.sender).send(26000000000));
            totalWithdrawForEvent += 26000000000;
            currentKeep = 0;
            isClaimedEvent[448] = true;
        } else {
            revert(string(abi.encodePacked("Can not withdraw right now")));
        }
    }
