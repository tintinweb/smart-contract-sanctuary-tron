...
//SourceUnit: CryptoBullSociety.sol
...
contract CryptoBullSociety is Ownable, ERC721, ReentrancyGuard {
  ...
  ) ERC721("CryptoBullSociety", "CryptoBullSociety", maxBatchSize_, collectionSize_) {
  ...
  {
    SaleConfig memory config = saleConfig;
    uint256 floorPrice = uint256(config.floorPrice);
    ...
    _safeMint(msg.sender, quantity);
    refundIfOver(floorPrice * quantity);
  }
  ...
  function setFloorPrice(uint64 floorPrice) external onlyOwner {
    saleConfig.floorPrice = floorPrice;
  }

  function getFloorPrice() public view returns (uint256) {
    return saleConfig.floorPrice;
  }
