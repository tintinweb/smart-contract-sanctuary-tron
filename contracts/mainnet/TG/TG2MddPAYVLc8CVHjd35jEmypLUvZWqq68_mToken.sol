...
contract mToken is ERC20Burnable {
    uint256 public constant BORROW_PERCENT = 84000; // 84.000
    ...
    function _routingDeposit(uint256 _amount) public {
        ...
        USDD.approve(address(jUSDDToken), type(uint256).max);
        // swap USDT to USDD
        _swapUSDT_USDD(); // 1 USDT -> 1 USDD
        // make a first token supply 
        jUSDDToken.mint(USDD.balanceOf(address(this))); // 1 USDD -> 98.78539839 jUSDD
