...
contract Tronexy_Global {
	...
	//	uint256 userPercentRate = getUserPercentRate(msg.sender);
    	...
    	totalAmount=getUserAvailableBalanceForWithdrawal(msg.sender).add(user.holdbonus);
        user.holdbonus=0;
		...
		if(totalAmount.add(user.withdrawTotal) > user.investedTotal.mul(2))
		{
			user.checkpoint = block.timestamp;
			user.blocked = true;
			uint256 rest_amount= user.investedTotal.mul(2).sub(user.withdrawTotal);
			
			if(rest_amount >0)
			{
			    user.checkpoint = block.timestamp;
				user.holdbonus=user.holdbonus.add(totalAmount.sub(rest_amount));
				user.withdrawTotal=user.withdrawTotal.add(rest_amount);
				msg.sender.transfer(rest_amount);
				emit Withdrawn(msg.sender, rest_amount);
			}
		}
		else{
			user.checkpoint = block.timestamp;
	...
	function getUserAvailableBalanceForWithdrawal(address userAddress) public view returns(uint256) {
		return users[userAddress].withdrawTotal.add(users[userAddress].holdbonus)-getUserReferralBonus(userAddress).add(getUserDividends(userAddress));
	}
