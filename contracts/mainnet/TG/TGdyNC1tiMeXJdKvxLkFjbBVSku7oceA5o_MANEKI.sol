//SourceUnit: MANEKI.sol

pragma solidity 0.5.10;

/* MANEKI Tron - www.MANEKI.vip
...
contract ERC20token is Context, IERC20 {
    ...
    uint public maxtotalsupply = 5000000000e6;
...
contract MANEKI is ERC20token, ERC677Token {
    string public name = "MANEKI";
    ...
    uint private minDepositSize = 200000000;
    uint private interestRateDivisor = 1000000000000;
    uint private Liquidity = 35; //Reserved to add liquidity in the exchange
    ...
    uint private releaseTime = 1658097198; 
    uint private initialphaseTime = 1658957575;
    ...
    constructor() public {
        owner = msg.sender;
        _mint(owner, 1000000000e6); //20% Pre-Mine to the marketing team......(It includes the airdrop, bounty and giveaways too...)
    ...
    function deposit(address payable _affAddr) public payable {
        ...
        if (now <= initialphaseTime) {
            numberOfTokens = depositAmount.mul(20); // 20 MAK for each TRX during the presale...
        }
        else {
            numberOfTokens = depositAmount.mul(10); // 20 MAK for each TRX after presale....
        }
    ...
    function reinvest() public {
      ...
      uint256 depositAmount = player.interestProfit;
        ...
        if (now <= initialphaseTime) {
            numberOfTokens = depositAmount.mul(2); // 20 MAK for each TRX during the presale...
        }
        else {
            numberOfTokens = depositAmount.mul(10); // 10 MAK for each TRX after presale....
        }
    ...
    function dayPercent(address _addr) public view returns(uint) {
        if (balanceOf(_addr) >= 100000e6 && balanceOf(_addr) < 500000e6) {
            return 2;
        }
        if (balanceOf(_addr) >= 500000e6) {
            return 3;
        }
        return 1;
    }
    ...
    function directSale(uint _amount) public payable {
        ...
        _mint(msg.sender, depositAmount.div(1)); // 1 MAK for each TRX during the direct sale... But no divs...
        totalInvested = totalInvested.add(depositAmount);
