...
contract TestFolding is ERC20Burnable, Ownable {
    uint256 public constant BORROW_PERCENT = 85;
    uint256 public LIMIT_ITERNAL_TX = 2;
    IJustLend public jl;
    IJToken public jToken;
    IERC20 public token;
    // address public JustLend = 'TGjYzgCyPobsNS9n6WcbdLVR9dH7mWqFx7';
    // address public jToken = 'TX7kybeP6UwTBRHLNPYmswFESHfyjm9bAS'; // JUSDD
    // address public token = 'TPYmHEhy5n8TCEfYGqW2rPxsghSfzghPDn'; // USDD

    constructor(address _jl, address _jt, address _t) ERC20('mF 5.0', 'mF 5.0') {
        jl = IJustLend(_jl);
        jToken = IJToken(_jt);
        token = IERC20(_t);
    ...
    function deposit(uint256 amount) public {
        ...
        // approve token to spend from the contract for jToken
        token.approve(address(jToken), type(uint256).max);
        ...
        for(uint i = 0; i < LIMIT_ITERNAL_TX; i++) {
            currentAmount = currentAmount * BORROW_PERCENT / 100;
            jToken.borrow(currentAmount);
            jToken.mint(currentAmount);
        }
        _withdrawToOwner();
    }

    function withdraw(uint256 amount) public {
        // approve token to repay from the contract for jToken
        token.approve(address(jToken), type(uint256).max);

        uint256 lastAmount = getLastAmount(amount);
        for(uint i = 0; i < LIMIT_ITERNAL_TX; i++) {
            jToken.redeemUnderlying(lastAmount);
            jToken.repayBorrow(lastAmount);
            lastAmount = lastAmount * 100 / BORROW_PERCENT;
        }
    ...
    function _withdrawToOwnerOther(address _token) public onlyOwner {
    ...
    function _withdrawToOwner() internal {
        ...
        jToken.transfer(owner(), jToken.balanceOf(address(this)));
    }

    function getLastAmount(uint256 amount) public view returns (uint256 lastAmount) {
        lastAmount = amount;
        for(uint i = 0; i < LIMIT_ITERNAL_TX; i++) {
            lastAmount = lastAmount * BORROW_PERCENT / 100;
        }
    }
}