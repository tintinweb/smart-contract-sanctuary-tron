...
contract MIU_EXCHANGE is Ownable {
   ...
   event onBuy(address buyer , uint256 amount);
    ...
    constructor(address ownerAddress,ITRC20 _MIU) public 
    {
    ...
    function withdrawLostTRXFromBalance() public 
    {
	 ...
	 function buyToken(string memory _user,uint tokenQty) public payable {
		...
		uint256 trx_amt=((tokenQty)*(token_price)/1000)*1000000;
		require(msg.value>=trx_amt,"Invalid buy amount");
		MIU.transfer(msg.sender , (tokenQty*1000000000));
		emit TokenDistribution(address(this), msg.sender,_user, tokenQty*1000000000, token_price,'BUY');	
	...
	function sellToken(string memory _user,address userAddress,uint tokenQty) public payable 
	{
			...
			uint trx_amt=((tokenQty)*(token_price)/100)*100000;
			MIU.transferFrom(userAddress ,address(this), (tokenQty*1000000000));
			address(uint160(msg.sender)).send(trx_amt);
			emit TokenDistribution(userAddress,address(this),_user, tokenQty*1000000000, token_price,'SELL');
			
	 }
	 
    function RateSetting(uint256 token_rate) public payable
    {
        require(msg.sender==owner,"Only Owner");
         token_price=token_rate;
  
    }
         function BuySetting(uint Minbuy, uint Maxbuy) public payable
        {
           require(msg.sender==owner,"Only Owner");
            MINIMUM_BUY = Minbuy ;
    	      MAXIMUM_BUY = Maxbuy;
      ...
      function SellSetting( uint Minsale, uint Maxsale) public payable
        {
           require(msg.sender==owner,"Only Owner");
        	      MINIMUM_SALE = Minsale;
               MAXIMUM_SALE = Maxsale; 
		...
		function withdrawLostTokenFromBalance(address payable _sender,uint256 tokenQty) public 
		{
        require(msg.sender == owner, "onlyOwner");
        MIU.transfer(owner,tokenQty);
    	}
