...
contract CHXToken is Context, ITRC20, Ownable {
    ...
    string private _name = 'CHXToken';
    string private _symbol = 'CHX';
    uint8 private _decimals = 6;
    uint256 private _totalSupply = 9136 * 10**uint256(_decimals);

    uint256 private minKeep = 1 * 10**uint256(5);
    ...
    uint256 public  MAX_STOP_FEE_TOTAL = 2136 * 10**uint256(_decimals);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFromFee[sender] || !_isExcludedFromFee[recipient]) {
            require(_balances[sender] >= amount.add(minKeep),"need leave 0.1 token");
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(
            ...
            uint256 inviterAmount = tAmount.div(100).mul(4);
            _takeInviterFee(sender, recipient, inviterAmount);

            _balances[recipient] = _balances[recipient].add(tTransferAmount.sub(inviterAmount));

            emit Transfer(sender, _exchangePool, tLiquidity);
            emit Transfer(sender, _fundAddress, tFund);
            emit Transfer(sender, _burnPool, tBurn);
            emit Transfer(sender, recipient, tTransferAmount.sub(inviterAmount));
        } else {
