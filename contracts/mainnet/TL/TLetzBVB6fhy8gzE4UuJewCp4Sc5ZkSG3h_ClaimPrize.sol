...
contract ClaimPrize is Ownable {
    ...
    function claims(
        address _liquidityProvider,
        uint256[] memory _tranche,
        uint256[] memory _balance,
        uint256[] memory _proofA,
        bytes32[] memory _merkleProof
    ) public {
        uint256 totalBalance = 0;
        uint256 startProof = 0;
        for (uint256 i = 0; i < _tranche.length; i++) {
            bytes32[] memory _proof = new bytes32[](_proofA[i]);
            for (uint256 j = startProof; j < startProof + _proofA[i]; j++) {
                _proof[j - startProof] = _merkleProof[j];
            }
            startProof = startProof.add(_proofA[i]);
            _claim(_liquidityProvider, _tranche[i], _balance[i], _proof);
            totalBalance = totalBalance.add(_balance[i]);
        }
