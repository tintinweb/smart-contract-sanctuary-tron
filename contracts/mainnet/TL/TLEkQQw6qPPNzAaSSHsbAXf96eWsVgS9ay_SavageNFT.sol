...
contract SavageNFT is TRC721, TRC721Enumerable, MinterRole,TRC721Metadata {
    INonces nonces;

    constructor(string memory _name,string memory _symbol,string memory _baseURI,address _noncesAddress) public TRC721Metadata(_name,_symbol) {
       _setBaseURI(_baseURI);
       nonces = INonces(_noncesAddress);
    }
    ...
    function verify(address nftAddress,address to,uint256 tokenId,string memory tokenURI,uint256 nonce,uint256 deadline,bytes32 r,bytes32 s,uint8 v) private view returns(bool) {
        bytes32 messageHash = keccak256(abi.encode(nftAddress,to,tokenId,tokenURI,nonce,deadline));
        bytes32 ethSignedMessageHash = getTronSignedMessageHash(messageHash);
        return isMinter(ecrecover(ethSignedMessageHash,v,r,s));
    }

    function permitMintWithTokenURI(address to, uint256 tokenId, string memory tokenURI,uint256 deadline,bytes32 r,bytes32 s,uint8 v) public onlyMinter returns (bool) {
        require(deadline > block.timestamp);
        uint256 nonce = nonces.getNonce(to);
        require(verify(address(this),to,tokenId,tokenURI,nonce,deadline,r,s,v),"未授权");
        _mint(to, tokenId);
        _setTokenURI(tokenId, tokenURI);
        nonces.add(to);
        return true;
    ...
    function getTokenIdByUser(address user) external view returns(uint256[] memory) {
