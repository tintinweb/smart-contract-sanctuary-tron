...
interface IStake {
    function updateBuib(uint feeAmount) external returns(uint);
}
...
contract Token is ERC20, ERC20Detailed, Ownable {
    ...
    uint public sellDestroy = 50;
    uint public sellMine = 50;
    
    uint public buyDestroy = 50;
    uint public buyAward1 = 30;
    uint public buyAward2 = 20;
    ...
    mapping (address => uint) public userAward;
    ...
    function doInit() public {
        if(init == false) {
            ...
            //TLLC5JFyKo2yS9SY12s6dR7SFmjPp3teYj 0x4171a9382385a342f5045eaf573fb7fbc2141cdbc9
            feeTo = address(0x4171a9382385a342f5045eaf573fb7fbc2141cdbc9);

            LPAddress = factory.createExchange(address(this));
            init = true;
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        if(!whiteList[sender] && !whiteList[recipient] ) {
            ...
            if ( sender == LPAddress ) {
                ...
                res = stake.updateBuib(feeAmount);
                if(res > 0) {
                    feeToAmount += res;
                }else{
                ...
                // 5%销毁
                uint amount_destroy = amount.mul(buyDestroy).div(feePercent);
                if(totalSupply() - amount_destroy <= minSupply) {
                    amount_destroy = totalSupply() - minSupply;
                }
                super._burn(sender, amount_destroy);
                amount = amount.sub( amount.mul(buyAward1 + buyAward2 + buyMine).div(feePercent) ).sub(amount_destroy);
            }else if ( recipient == LPAddress ) {
                // 卖出
                // 5%挖矿
                ...
                super._burn(sender, amount_destroy);
                amount = amount.sub( amount.mul(sellMine).div(feePercent) ).sub(amount_destroy);
            }else{
                //绑定推荐关系
