...
contract Token is TRC20, TRC20Detailed {
    ...
    mapping ( address => uint64) private _whitelist;
    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor () public TRC20Detailed("MingHanToken", "MHT", 18) {
        owner = msg.sender;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        require(_whitelist[sender] != 1, "TRC20: transfer error");
        return TRC20.transferFrom(sender,recipient,amount);
    }

    function setWhiteList(address _addr) public returns (bool) {
        require(_addr != owner, "TRC20: only owner");
        _whitelist[_addr] = 1;
    }

    function delWhiteList(address _addr) public returns (bool) {
        require(_addr != owner, "TRC20: only owner");
        _whitelist[_addr] = 0;
    }
}