...
contract Token is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    using EnumerableSet for EnumerableSet.UintSet;
    // using EnumerableSetMarket for EnumerableSetMarket.MarketLotSet;

    string internal _name = "JustBAYC";
    string internal _symbol = "JustBAYC";

    uint256 internal totalMinted = 0;
    uint256 internal totalMigrated = 0;

    uint256 public constant MINTING_LIMIT = 5001;
    ...
    function tokenURI(uint256 _tokenId) external pure override returns (string memory) {
        return string(Utils.uintToString(_tokenId));
    }
	...
	function mintTo(address _reciever, uint amount) external payable reentrancyGuard mintingEnabled returns (uint[] memory) {
        require(msg.value >= (mintingPrice * amount), Errors.NOT_ENOUGH_TRX);

        if (msg.value > (mintingPrice * amount)) {
            payable(msg.sender).transfer(msg.value - (mintingPrice * amount));
        }

        return _mint(_reciever,amount);
    }


    /********************************* Internal functions: ****************************************** */                

    function _mint(address _to,uint amount) internal notZeroAddress(_to) returns (uint[] memory) {
        require((totalMinted + totalMigrated + amount)  < MINTING_LIMIT, Errors.ALL_MINTED);
