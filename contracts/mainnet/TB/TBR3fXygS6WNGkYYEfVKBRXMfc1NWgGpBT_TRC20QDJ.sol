...
contract TRC20QDJ is Context, IBEP20, Ownable {
  ...
  //lp矿池手续费
  uint public uniswapbuyfee=5;
  uint public uniswapsalefee=5;
  address public uniswapV2Pair;

  mapping(address=>bool) private _whitearr;
 
  //黑洞钱包
  address public burnacc = address(0x410000000000000000000000000000000000000000);
  //收款钱包
  address public mainacc = address(0x412E91ED75E62251FBD34276D0ABB355B96BF1D538);
  //理事会钱包
  address public lshacc = address(0x41EF878755C3B8D6D91F38258DF19AE30D327CBF76);
  //布道基金
  address public bdjjacc = address(0x4167B1AC71F5E4CC11D1CC2509F4A2FCE520A6C287);

  constructor() public {
    _name = "QDJ";
    _symbol = "QDJ";
    _decimals = 8;
    _totalSupply = 10000 * 10**8;
    maxuid=0;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(sender==uniswapV2Pair){
        if(_balances[burnacc]<7000*10**8){
           _tokenTransfer(sender,burnacc,amount.mul(2).div(100));
           _tokenTransfer(sender,lshacc,amount.mul(1).div(100));
           _tokenTransfer(sender,bdjjacc,amount.mul(2).div(100));
        }else{
           _tokenTransfer(sender,lshacc,amount.mul(2).div(100));
           _tokenTransfer(sender,bdjjacc,amount.mul(3).div(100));
        }
