...
contract TronStep {
	...
	function withdraw() public {
		...
		uint256 totalAmount;
		uint256 dividends;
		uint256 userPercentage=getContractBalanceRate();
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(220).div(100)) {
					...
					dividends = (user.deposits[i].amount.mul(userPercentage).div(PERCENTS_DIVIDER))
...
// 		}
	...
	function Reinvest(uint256 _value) internal {
		owner.transfer(_value.mul(100).div(PERCENTS_DIVIDER));
		User storage user = users[msg.sender];
