...
library SafeMath {
    ...
    contract FMDAOGame {
        ...
        function buyLotto(uint count, address addr, uint8 buyType) external {
            ...
            require(count <= 50, "The number is not greater than 50");
            getLottoCode(addr, count, buyType);
        }

        function getLottoCode(address addr, uint count, uint8 buyType) internal {
            if(count == 0) {
                return;
            }
            ...
            uint usdtAmount = count * 10 * 10 ** 6;
            lottery.totayLotteryAmountTotal = SafeMath.add(lottery.totayLotteryAmountTotal, usdtAmount);
            if(buyType == 1){
                uint lotteryPool = SafeMath.div(SafeMath.mul(usdtAmount, 3), 10);
        ...
        function deposit(uint usdtVal, address addr) external returns(uint) {
            ...
            DappDatasets.Lottery storage lottery = lotteryGame[lotterySession];
            depositBalance = SafeMath.div(SafeMath.mul(usdtVal, 80), 100);
            fomo.participant.push(addr);

            uint lotteryPool = SafeMath.div(usdtVal, 20);
            lottery.lotteryPool = SafeMath.add(lottery.lotteryPool, lotteryPool);
            lottery.todayAmountTotal = SafeMath.add(lottery.todayAmountTotal, lotteryPool);
