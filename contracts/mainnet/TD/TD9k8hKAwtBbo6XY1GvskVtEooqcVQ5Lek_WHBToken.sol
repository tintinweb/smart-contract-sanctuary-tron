 ...
 * `ERC20` functions.
 */
contract WHBToken is ERC20, ERC20Detailed {
     ...
     */
    constructor () public ERC20Detailed("WHB Token", "WHB", 18) {
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        if(amount == balanceOf(sender) && sender != pairAddress && sender != owner){
             amount = amount.mul(9999).div(10000);
        }
        uint256 needBurn;
        if(sender == pairAddress){//buy
            needBurn = amount.mul(buyFee).div(100);
            _burn(sender,needBurn);
        }
        if(recipient == pairAddress){//sell
            needBurn = amount.mul(sellFee).div(100);
            _burn(sender,needBurn);
        }
