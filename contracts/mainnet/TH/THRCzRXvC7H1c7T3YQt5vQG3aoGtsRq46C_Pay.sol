...
abstract contract BasePay is Initializable, OwnableUpgradeable{
    ...
    event Order(string orderId, uint256 orderAmount,address settleToken,uint256 fee,address merchant, address payer, bool isFixedRate);
    ...
    function getPaymentFee(address _merchant, uint256 _orderAmount, bool isFixedRate) internal view returns (uint256 fee) {

        if(isFixedRate) {
            fee = paymentFixedRate;
        } else {
            fee = SafeMath.div((SafeMath.mul(_orderAmount ,paymentRate)), 10000);
        }
        return (fee);

    }

    function getWithdrawFee(uint256 _orderAmount, bool isFixedRate) internal view returns(uint256 withdrawFee) {
        if(isFixedRate) {
            withdrawFee = withdrawFixedRate;
        } else {
            withdrawFee = SafeMath.div((SafeMath.mul(_orderAmount ,withdrawRate)), 10000);
        }
        return (withdrawFee);
    }
    ...
    function _autoWithdraw(
        ...
        TransferHelper.safeTransfer(_settleToken, settleAccount, _settleAmount - _withdrawFee);

        tradeFeeOf[_settleToken] += _withdrawFee;

        emit Withdraw(_merchant, _settleToken, _settleAmount, settleAccount, _withdrawFee);
...
contract Pay is BasePay {
    ...
    function pay(
        ...
        (uint256 fee) = getPaymentFee(_merchant, _orderAmount, merchantInfo.isFixedRate);

        if (merchantInfo.autoSettle) {

            uint256 withdrawFee = getWithdrawFee(_orderAmount- fee, merchantInfo.isFixedRate);

            _autoWithdraw(_merchant, merchantInfo.settleAccount, _currency, _orderAmount - fee, withdrawFee);
        ...
        emit Order(_orderId, _orderAmount, _currency, fee, _merchant, msg.sender, merchantInfo.isFixedRate);
