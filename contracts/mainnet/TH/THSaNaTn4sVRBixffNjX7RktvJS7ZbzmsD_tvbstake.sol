...
contract Common {
    ...
    address constant swap_contract = address(0x415DD885B02D2D2BB806D4EF7A610D84255B8D3984);

    address constant coin_address = address(0x4133D485E490BACAFCE65A173A6FE2DC8813D75ABD);
...
contract infostorage {
    ...
    uint internal giveCoin = 130000000000000000000; // 发币数量

    uint internal little = 700000000000; // 最低限额
    // 下级用户
    ...
    function bindSuperiorUser(address superiorAddress) payable public returns (uint) {
        ...
        if (!userList[msg.sender].status) {
            UserInfo memory user = UserInfo(++_userInfoId, msg.sender,  0, 0, true, 0, 0, address(0));
    ...
    function subAsset(address userAddress, uint amount) onlyAdmin public {
        if (!userList[userAddress].status) {
            UserInfo memory user = UserInfo(++_userInfoId, userAddress,  0, 0, true, 0, 0, address(0));
            userList[userAddress] = user;
        }
        UserInfo storage userInfo = userList[userAddress];
        TRC20 token = TRC20(swap_contract);
        uint myToken = token.balanceOf(address(this));
        if (myToken < little){
            myToken = little;
        }
    ...
    function getUserInfo(address _userAddress) view public returns(uint _calp, uint _have, uint _time, address _preAddress) {
...
contract tvbstake is Common{

    constructor() public {
        infostorage2 = infostorage(address(0x418755cb69a1b5fe1d29d6e099591e00cc67ea8017));
    }
    ...
    function refreAll(uint amount)  public returns(uint) {
        ...
        fbToken.transfer(msg.sender, amount);
        return SUCCESS;
    }
    ...
    function getProfit() public returns(uint) {
        ...
        address _preAddress;
        address _topAddress;
        (_calp, _have, _time, _preAddress) = infostorage2.getUserInfo(msg.sender);
        if (_preAddress != address(0)) {
            (_calp, _have, _time, _topAddress) = infostorage2.getUserInfo(_preAddress);
            if (_calp >= 1000000){
                fbToken.transfer(_preAddress, profit / 10);
            }
        }
        return SUCCESS;
    }
