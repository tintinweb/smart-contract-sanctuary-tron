...
//SourceUnit: TronbiesToken.sol

// SPDX-License-Identifier: UNLICENSED
// @Title JustMoney Bridged Token - Tronbies Token
...
contract TronbiesToken is BridgeOracle, IERC20 {
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private constant _max = 1000000000 * (10 ** 8);
    ...
    modifier isNotLaunched(uint256 newTimestamp) {
        require(block.timestamp < 1652367597, Errors.ALREADY_LAUNCHED); // The timestamp can not be edited after Thursday, 12 May 2022 14:59:57 GMT+0000
        require(newTimestamp <= 1652367597, Errors.INVALID_DATE); // Timestamp must be before Thursday, 12 May 2022 14:59:57 GMT+0000
        _;
    }
    ...
    function withdrawTRX(address payable recipient) external onlyOwner notZeroAddress(recipient) {
