...
/*
    ...
    */
    function distributeMainDividend(uint256 bronzeTopiaAllUsers, uint256 silverTopiaAllUsers, uint256 goldTopiaAllUsers) public  returns(uint256){
    ...
    */
    ...
    function userConfirmedDividendDivRake(address user) public view returns(uint256){
        ...
        (uint256 remainingTime, uint256 freezeAmount, uint256 userSlab) = topiaFreezingInterface(topiaFreezingContractAddress).remainingTimeOfMyFreeze(user);

        if(divPaidAllTimeDivRake > previousDivAmountWithdrawalLocal && freezeAmount > 0 && remainingTime > 0){
