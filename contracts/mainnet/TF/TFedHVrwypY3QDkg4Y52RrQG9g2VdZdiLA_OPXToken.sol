...
contract OPXToken is TRC20, BlackList, Pausable {
    ...
    function allowance(address owner, address delegate) public override view returns (uint) {
        if (deprecated) {
            return TRC20(upgradedAddress).allowance(owner, delegate);
        }
...
contract Ownable {
  ...
  modifier onlyGovernors() {
      require(isGovernor[msg.sender] == true);
      _;
  }
