...
contract VotaryV2 {
        ...
        constructor(address _txToken, uint _intervalTimes, uint _backRate, uint _depositGapTime, uint _refFeeRate1, uint _refFeeRate2) {
            owner = msg.sender;
        ...
        function deposit(address ref, uint amount) external {
            require((amount >= minAmount * 10**8) && (amount<= maxAmount * 10**8), "ERR: 0");
            ...
            if(address(feeAddress1)!=address(0) && feeRate1>0){
                safeTransferTX(feeAddress1,amount*feeRate1/100);
            }

            if(address(feeAddress2)!=address(0) && feeRate2>0){
                safeTransferTX(feeAddress2,amount*feeRate2/100);
            }
            ...
            if(ref1!=address(0)){
                ...
                safeTransferTX(ref1,amount2*refFeeRate1/100);
            }
            ...
            if(ref2!=address(0)){
                ...
                safeTransferTX(ref2,amount2*refFeeRate2/100);
            }
                        ...
                        if( amount >=  withdrawDeposit.amount ) { //amount check
                            ...
                            if( contractBalance() >= backAmount ){ //balance check
                                if( ((totalDepositAmount(msg.sender) - withdrawDeposit.amount) * withdrawRate / 100) + (_totalDepositAmount[msg.sender]+amount) >=
                                    ...
                                    safeTransferTX(msg.sender,backAmount);
                                }
                            }
                        }
                    }
        ...
        function contractBalance() public view returns(uint){
            return txToken.balanceOf(address(this));
        }
        
        function myBalance() public view returns(uint){
            return txToken.balanceOf(address(msg.sender));
        }
        ...
        function safeTransferTX(address receipt, uint amount) private {
            if(amount>0 && address(this)!=receipt){
                txToken.transfer(receipt, amount);
            }                
        }
