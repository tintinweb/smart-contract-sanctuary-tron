...
contract AMBLE is TRC721, TRC721Enumerable, TRC721MetadataMintable,BlackLkisted {
    using SafeMath for uint256;
	IBEP20 dart;
	constructor() public TRC721Metadata('AMBLE', 'AMBLE') {
		sellingFess = 5;
	}
    ...
    function buyWithTokens(uint256 tokenId,address payable _reference,uint256 amount) internal {
        ...
        _transferFrom(_owner,msg.sender,tokenId);
        nftINFO[tokenId].sell=false;
        nftINFO[tokenId].owner_address=msg.sender;
	}
	...
	function buyWithBNB(uint256 tokenId,address payable _reference) public payable {
		...
		address payable _owner = address(uint160(ownerOf(tokenId)));
		_owner.transfer((msg.value * owner_fees) / (100));
		address(uint160(owner())).transfer((msg.value * sellingFess) / (100));
		address(uint160(nftINFO[tokenId].creator_address)).transfer((msg.value * nftINFO[tokenId].royalty) / (100));
        _transferFrom(_owner,msg.sender,tokenId);
        nftINFO[tokenId].sell=false;
        nftINFO[tokenId].owner_address=msg.sender;
	}
