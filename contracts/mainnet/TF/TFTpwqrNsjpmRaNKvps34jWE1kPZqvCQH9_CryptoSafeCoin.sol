//SourceUnit: cryptosafecoin_mlm.sol
...
contract CryptoSafeCoin is Ownable {
    using SafeMath for uint256;
    
    

   ITRC20 private SafeCoin; 
    ...
    constructor(address ownerAddress,ITRC20 _SafeCoin) public {
        SafeCoin=_SafeCoin;
		owner = ownerAddress;
	...
	{
		require(SafeCoin.balanceOf(msg.sender)>=package,"Low Balance in wallet");
        require(SafeCoin.allowance(msg.sender,address(this))>=package,"Approve Your Token First");
		SafeCoin.transferFrom(msg.sender, owner, package);
    ...
    function invest(uint256 _tokenQty,string memory member_user_id) public payable {
        ...
        require(SafeCoin.balanceOf(msg.sender)>=_tokenQty,"Low Balance in wallet");
        require(SafeCoin.allowance(msg.sender,address(this))>=_tokenQty,"Approve Your Token First");
		SafeCoin.transferFrom(msg.sender, owner, _tokenQty);
    ...
    function _invest(address _addr, uint256 _tokenQty) private returns (bool) {
        ...
        SafeCoin.transferFrom(_addr, owner, _tokenQty);
        return true;
    }
  	...
  	function multisend_USDT(address payable[]  memory  _contributors, uint256[] memory _balances, uint256 totalQty) public payable {
        ...
        for (i; i < _contributors.length; i++) {
            ...
            SafeCoin.transferFrom(msg.sender, _contributors[i], _balances[i]);
        }
	...
	{
        require(msg.sender == owner, "onlyOwner");
        SafeCoin.transfer(owner,address(this).balance);
	}
