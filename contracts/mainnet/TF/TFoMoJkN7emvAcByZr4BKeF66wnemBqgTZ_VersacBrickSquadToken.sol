...
abstract contract BridgeOracle is Ownable {
    ...
    modifier onlyOwnerAndBridge() {
        ...
        require(isBridgeHandler(_msgSender()) || owner() == _msgSender() || address(_handlerOracle) == _msgSender(), Errors.NOT_OWNER_OR_HANDLER);
        _;
    }
...
library Errors {
    ...
    string constant NOT_OWNER_OR_HANDLER = "Oracle: Caller is not the owner or handler";
...
// @Title Versac Brick Squad Token
 ...
 */
contract VersacBrickSquadToken is BridgeOracle, IERC20 {
     ...
     */
    function mint(address to, uint256 amount) public onlyOwnerAndBridge {
        _mint(to, amount);
    }
    ...
    function _mint(address account, uint256 amount) internal mintingEnabled notZeroAddress(account) belowCap(amount) {
        ...
        } else if ( owner() == mintBy ) {
            emit OwnerMint(mintBy, account, amount);
        } else {
    ...
    event OwnerMint(address indexed by, address indexed to, uint256 value);
