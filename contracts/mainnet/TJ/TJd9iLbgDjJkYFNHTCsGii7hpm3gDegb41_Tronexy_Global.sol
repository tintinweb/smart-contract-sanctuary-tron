...
contract Tronexy_Global {
	...
	uint256[] public REFERRAL_PERCENTS = [500, 20, 10, 10, 10, 5, 5, 5, 5, 5];
	...
	function getUserAvailableBalanceForWithdrawal(address userAddress) public view returns(uint256) {
		return getUserReferralBonus(userAddress).add(getUserDividends(userAddress))-users[userAddress].withdrawTotal.add(users[userAddress].holdbonus);
	}
