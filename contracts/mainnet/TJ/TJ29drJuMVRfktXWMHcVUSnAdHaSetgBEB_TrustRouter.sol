...
//SourceUnit: ITrustRouter01.sol

// SPDX-License-Identifier: MIT
pragma solidity >=0.8.0;

interface ITrustRouter01 {
...
//SourceUnit: ITrustRouter02.sol

// SPDX-License-Identifier: MIT
pragma solidity >=0.8.0;
import "./ITrustRouter01.sol";

interface ITrustRouter02 is ITrustRouter01 {
...
//SourceUnit: TrustRouter.sol

// SPDX-License-Identifier: MIT
pragma solidity >=0.8.0;

import "./SafeMath.sol";
import "./ITrustRouter02.sol";
...
contract TrustRouter is ITrustRouter02 {
    ...
    function _addLiquidity(
        ...
        address pair=ITrustFactory(factory).getPair(tokenA, tokenB);
        if ( pair== address(0)) {
            pair=ITrustFactory(factory).createPair(tokenA, tokenB);
        }
    ...
    function removeLiquidityWithPermit(
        ...
        address pair=ITrustFactory(factory).getPair(tokenA, tokenB);
        uint value = approveMax ? 1e40 : liquidity;
        ITrustPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'TrustRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ITrustFactory(factory).getPair(path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'TrustRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ITrustFactory(factory).getPair(path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(ITrustFactory(factory).getPair(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'TrustRouter: INVALID_PATH');
        amounts =  getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, 'TrustRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ITrustFactory(factory).getPair(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
