...
contract Nsgswap {
    ...
    uint256 private constant starPercents = 50;
    ...
    uint256 private constant minDeposit = 25e6;
    ...
    uint[] level_bonuses = [600, 200, 300, 400, 200, 100, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];  
    ...
    function deposit(uint256 _amount) external {
        _deposit(msg.sender, _amount,0, false);
        emit Deposit(msg.sender, _amount);
    }
    function _deposit(address _user, uint256 _amount, uint8 _isReDept, bool isSplit) private {
        require(_amount>=minDeposit && _amount<=maxDeposit && _amount.mod(minDeposit) == 0, "Minimum 25 And Multiple 50");
        ...
        if(userInfo[_user].myLastDeposit==0){
            userInfo[userInfo[_user].referrer].myActDirect+=1;
        }
        ...
        if(_amount>=200e6 && userInfo[_user].incomeArray[9]>=200e6){
            userInfo[_user].totalIncome+=200e6;
            userInfo[_user].incomeArray[9]-=200e6;
            userInfo[_user].aftFreez=2000e6;
        }else{
            unfreezeDepts(_user);
        }
    ...
    function _setReferral(address _user,address _referral, uint256 _refAmount, uint8 _isReDept) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            if(userInfo[_referral].isLeader==0 || userInfo[_referral].isManager==0){
                ...
                if(userInfo[_referral].isLeader==0 && (ltA+ltB)>=50 && userInfo[_referral].myLastDeposit>=1000e6 && lbA>=10000e6 && lbB>=10000e6){
                   userInfo[_referral].isLeader=1;
                   reward_array.push(_referral);
                }
                if(userInfo[_referral].isManager==0 && (ltA+ltB)>=200 && userInfo[_referral].myLastDeposit>=1000e6 && lbA>=50000e6 && lbB>=50000e6){
            ...
            if(i==0){
                ...
                if(userInfo[_referral].myActDirect>=5){
                ...
                if(userInfo[_referral].isLeader==1 && i >= 3 && i < 5){
                    userInfo[_referral].incomeArray[9]+=levelOn.mul(level_bonuses[i]).div(baseDivider);
    ...
    function depositBySplit(uint256 _amount) external {
        require(_amount >= minDeposit && _amount.mod(minDeposit) == 0 && _amount<=maxDeposit, "amount err");
    ...
    function unfreezeDepts(address _addr) private {
        ...
        for(uint i=0;i<userDepts[_addr].length;i++){
            ...
            if(pl.isUnfreezed==false && block.timestamp>=pl.unfreeze && isdone==0){
                ...
                userInfo[_addr].totalIncome+=pl.amount.mul(160).div(1000);
                userInfo[_addr].incomeArray[0]+=pl.amount;
                userInfo[_addr].incomeArray[1]+=pl.amount.mul(160).div(1000);
                isdone=1;
    ...
    function withdraw(uint256 _amount) public{
        ...
        uint256 tempSplit=(bonus-player.incomeArray[0]).mul(25).div(100);
        ...
        if(player.aftFreez>=player.incomeArray[5]){
            player.aftFreez-=player.incomeArray[5];
            player.incomeArray[5]=0;
        }else{
            player.incomeArray[5]-=player.aftFreez;
            player.aftFreez=0;
