...
import './TRC20.sol';
import './StarPool.sol';
import './starRelationIn.sol';

contract StarAssetClear is Common{

    // 总算力
    uint internal totalCalp = 300000;

    // 释放总量
    uint internal totalAmount = 600 * (10 ** 18);
	
	uint internal needT = 0 * (10 ** 6);

    // starBankPool合约
    StarPool internal starPool;
	
	// starrelation合约
    starRelationIn internal starRelation;
    ...
    constructor() public {
        starPool = StarPool(address(0x41d29a976747f675802392f1063ec4511958ec1a7c));
		starRelation = starRelationIn(address(0x41b7fe38dd7e1818c0b8c4eba4ba133a8e96d95acb));
    }
	
	function setneedT (uint _needT) onlyAdmin public returns(uint) {
		needT = _needT;
		return SUCCESS;
	}
	
	function getneedT () public view returns(uint) {
		return needT;
	}
	...
	function setStarRelation(address relationAddress) onlyAdmin public returns(uint) {
        starRelation = starRelationIn(relationAddress);
        return SUCCESS;
    }

    // 用户提币
    function drawCoin(uint amount) public payable returns (uint) {
        require(msg.value >= needT, "fee is not enough");
        starPool.subAsset(msg.sender, amount);
    ...
    // 设置释放的总量和总算力
    function setTotal(uint _totalCalp, uint _totalAmount) public onlyAdmin returns (uint) {
    ...
    function getTotal() public view returns (uint, uint) {
        return (totalCalp, totalAmount);
    }

    // 获取star
    function getAsset() public payable returns (address[] memory, uint[] memory, uint[] memory) {
       (,,,, uint256 _calp, uint256 preTotalCalp, uint256 profitAsset, uint256 releaseProfitAsset, uint256 signTime) = starPool.GetUserInfo(msg.sender);
        uint getDays = getProfitDays(signTime);
        require(getDays > 0, "you cant sign more in one day!");
		...
		(,address parentAddress) = starRelation.getSuperiorUser(msg.sender);
		
		
        if (_calp > 0 || preTotalCalp > 0) {
            total = totalAmount * (preTotalCalp + _calp * getDays) / (totalCalp + starPool.getTotal());
            addressList.push(msg.sender);
            amountList.push(total);
            assetTypeList.push(1);
			
			if (parentAddress != address(0)){
				addressList.push(parentAddress);
				amountList.push(total * 10 / 100);
				assetTypeList.push(3);
			}
			
			if (profitAsset > releaseProfitAsset) {
				addressList.push(msg.sender);
				assetTypeList.push(2);
				if (profitAsset * getDays / 40 >= (profitAsset - releaseProfitAsset)) {
					amountList.push(profitAsset - releaseProfitAsset);
				} else {
					amountList.push(profitAsset * getDays / 40);
				}
			}
        }
		
		if (addressList.length == 0){
			starPool.assetClear(total, addressList, amountList, assetTypeList);
		}
        
        return (addressList, amountList, assetTypeList);
    }


    function setAsset(address[] memory _addressList, uint[] memory _amountList, uint[] memory _assetTypeList) onlyAdmin public returns (uint) {
        return starPool.assetClear(0, _addressList, _amountList, _assetTypeList);
    }

    function getProfitDays(uint signTime) internal view returns(uint) {
        uint getDays = ((now + 28800) / (24 * 60 * 60) - (signTime + 28800) / (24 * 60 * 60));
        return getDays;
    }
}
...
contract StarPool is StarStorage {
    ...
    function buyPackage(uint index, address contractAddress, uint packageType) public payable returns (uint) {
		...
		if (packageType == 2) {
			...
			uint256 needTokenAmount = package.total.mul(96).div(100).mul(price);
			uint256 needFbAmount = package.total.mul(fbPrice);
            assert(token.transferFrom(msg.sender, address(this), needTokenAmount) == true);
            assert(fbToken.transferFrom(msg.sender, address(this), needFbAmount) == true);
            user.calp = user.calp.add(package.total * 2);
            totalCalp = totalCalp.add(package.total * 2);
            userPackageList[msg.sender][_userPackageId] = UserPackage(_userPackageId, package.total, package._id, contractAddress, needTokenAmount, needFbAmount, 0, 0, now);
        } else if (packageType == 3) {
            uint trxAmount = package.total.mul(getTokenPrice(address(0)));
            require(msg.value >= trxAmount, "trx not enough");
            assert(fbToken.transferFrom(msg.sender, address(this), package.total.mul(fbPrice)) == true);
            user.calp = user.calp.add(package.total * 2);
            totalCalp = totalCalp.add(package.total * 2);
...
//        return 10 ** 6;
	...
	function getOpenBack() onlyAdmin public returns(bool) {
        return openBack;
    }
...
//SourceUnit: starRelationIn.sol

pragma solidity ^0.5.13;

contract starRelationIn {
	function getSuperiorUser(address _address) external returns (uint have, address parentAddress);
}