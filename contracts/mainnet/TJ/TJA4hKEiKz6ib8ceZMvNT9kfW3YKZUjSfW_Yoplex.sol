...
contract Yoplex is ERC20, ERC20Burnable, Ownable{
    ...
    uint public total_invest_trx = 0;
    uint public total_invest_usd = 0;
    uint public total_roi_withdrawal_trx = 0;
    uint public total_roi_withdrawal_usd = 0;
    uint public total_withdrawal_yoplex = 0;
    uint public total_withdrawal_usd = 0;
    uint public admin_withdrawal_trx = 0;
    uint public admin_withdrawal_usd = 0;
	uint private withdrawal_fee_in_lock = 5;
	uint private withdrawal_fee_after_lock = 1;
	uint private lock_period = 30 days;
	uint private token_price = 50000;  // 0.05 USD
	uint private TRX_price = 76000;
	uint[] private min_balance = [50000000, 5000000000, 15000000000];
	uint[] private ROI_percent = [25, 35, 50];
    uint[] private level = [3, 3, 6, 9, 12, 15];
    ...
    constructor(address _account) ERC20("Yoplex", "Yoplex") {
        ...
        userStruct = UserStruct({
			...
			investment_time: block.timestamp + 14 days,
			ROI_percent: 2,
			ROI_before_investment: 0,
            ROI_hold: 0,
			ROI_taken_time: block.timestamp + 14 days,
            ROI_taken: 0,
            withdrawal: 0,
            withdrawal_time: block.timestamp + 14 days,
            ROI: [[uint(0),uint(0),uint(0)],[uint(0),uint(0),uint(0)],[uint(0),uint(0),uint(0)]],
            level: 0,
            gen: [uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0),uint(0)]
        });
    ...
    function regUser(address payable _referrerID) public payable {
        ...
        require(TRX_to_USD(msg.value) >= min_balance[0], "Register with minimum 50 USD");
    ...
    function genCount(address _user, uint _gen) internal {
    ...
    function viewUserHoldROI(address _user) public view returns(uint) {
        uint ROI = 0;
        for (uint i = 0; i < 3; i++) {
            if(users[_user].referral.length < 5){
                ROI += users[_user].ROI[i][1] * ROI_percent[i] * ((block.timestamp - users[_user].ROI_taken_time) / 1 days) / 10000 / 10;
    ...
    function ROI_Withdrawal() public returns (bool) {
        ...
        if(users[msg.sender].referral.length >= 5 && users[msg.sender].level > 0){
    ...
    function viewGen(address _user) public view returns(uint[21] memory){
        return users[_user].gen;
    }

    function viewTotalInvestment(address _user) public view returns(uint){
        uint amount = 0;
        for (uint i = 0; i < 3; i++) {
            for (uint j = 0; j < 3; j++) {
