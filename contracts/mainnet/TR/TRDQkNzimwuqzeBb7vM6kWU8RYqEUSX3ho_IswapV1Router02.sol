//SourceUnit: IswapV1Router (4).sol
...
contract IswapV1Router02 is IIswapV1Router02 {
	...
	address[] memory path = new address[](pathAddr.length);
        ...
        uint amountTRXout = amounts[amounts.length - 1];
        uint amountTRXFeeout = amountTRXout.sub(amountTRXout / uint(2000)); // calcuate fee trx
        IWTRX(WTRX).withdraw(amountTRXFeeout);
        TransferHelper.safeTransferTRX(to, amountTRXFeeout);
    }
    function swapExactTokensForTRX(uint amountIn, uint amountOutMin, uint256[] calldata pathAddr, address to, uint deadline)
        external
        virtual
