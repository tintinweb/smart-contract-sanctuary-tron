...
contract BabyFoxCoin is TRC20 , Context , Ownable{
    ...
    constructor() {
        ...
        maxBuyAmountPerTxn = 50000 * 10**_decimals;
        
        maxTransferPercentagePerDay =10 ;
    }
    ...
    function _transfer( address sender,address recipient, uint256 amount) internal {
        ...
        if (liquidityWallet[recipient] && !_isExcludedFromFee[sender])
        {
            ...
            if(block.timestamp < _firstSell[sender]+24 * 1 hours){
                require(_totSells[sender]+amount <= _maxSellperday[sender], "You can't sell more than maxSell PerDay");
               _totSells[sender]=_totSells[sender].add(amount);
            }
            else{
                require(amount <= _maxSellperday[sender],"You can't sell more than maxSell PerDay");
        ...
        if(isBuy)
        {
           ...
           transferAmount =  amount.mul(98).div(100);
           rewardAmount = amount.mul(2).div(100);
