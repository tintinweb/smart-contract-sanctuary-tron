...
contract SafeMoney is SafeMoneyBase  {
	...
	uint256 private _totaltrxAsSMClaimed; // The total number of trx that was converted to SM and claimed by all addresses
	mapping(address => uint256) private _trxRewardClaimed; // The amount of trx claimed by each address
	mapping(address => uint256) private _trxAsSMClaimed; // The amount of trx converted to SM and claimed by each address
	...
	bool private _reimburseAfterSMClaimFailure; // If true, and SM reward claim portion fails, the portion will be given as trx instead
 ...
 function initialize() public initializer {
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim SM tokens
		if (!claimSM(user, claimtrxAsTokens)) {
	...
	function claimSM(address user, uint256 trxAmount) private returns (bool) {
		if (trxAmount == 0) {
			return true;
		}
		...
		_trxAsSMClaimed[user] += trxAmount;
		_totaltrxAsSMClaimed += trxAmount;
		return true;
	}
    ...
    function trxRewardClaimedAsSM(address byAddress) public view returns (uint256) {
		return _trxAsSMClaimed[byAddress];
	}
    ...
    function totaltrxClaimedAsSM() public view returns (uint256) {
		return _totaltrxAsSMClaimed;
	}
...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
    ...
    function __Ownable_init_unchained() internal initializer {
        ...
        _owner = 0x623dE6aC32B641E173ee04D477F50988A5c11256;
        emit OwnershipTransferred(address(0), 0x623dE6aC32B641E173ee04D477F50988A5c11256);
    }
...
abstract contract SafeMoneyBase is
    ITRC20Metadata,
    ReentrancyGuard,
    Initializable,
    ContextUpgradeable,
    OwnableUpgradeable
{
    ...
    string private constant NAME = "SafeMoney";
    string private constant SYMBOL = "SM";
    ...
    // FEES & REWARDS
    bool private _isSwapEnabled=true; // True if the contract should swap for liquidity & reward pool, false otherwise
    ...
    function TestToken_v2_init_base(address sendTO) internal initializer {

        _balances[sendTO] = totalSupply();
        ...
        _addressesExcludedFromFees[sendTO] = true;
...
//SourceUnit: proxyUpgradeableContract.sol
...
contract Initializable {
 ...
 contract ProxyUpgradeableContract is  Initializable, ITRC20Metadata,Ownable {
    ITRC20Metadata public token;

function initialize(address _token) public onlyOwner {
        token = ITRC20Metadata(_token);
    }
...
//SourceUnit: proxyUpgradeableV2.sol
