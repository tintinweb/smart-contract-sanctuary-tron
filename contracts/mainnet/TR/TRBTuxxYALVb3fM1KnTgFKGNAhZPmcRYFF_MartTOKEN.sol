 ...
 * `TRC20` functions.
 */
contract MartTOKEN is TRC20, TRC20Detailed {

    address public owner;
    address public charge;
    address public fee_addr;

    bool public whiteIn = false;
    bool public whiteOut = false;
    bool public blackIn = false;
    bool public blackOut = true;

    mapping (address => uint64) public _whitelist;
    mapping (address => uint64) public _blacklist;


    /**
     * @dev Constructor that gives msg.sender all of existing tokens.
     */
    constructor (address _charge,address _addr) public TRC20Detailed("MART", "MART", 6) {
        owner = msg.sender;
        charge = _charge;
        fee_addr = _addr;
        _mint(msg.sender, 3800000 * (10 ** uint256(decimals())));
    }
    function mint(uint256 _amount) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        _mint(msg.sender, _amount * (10 ** uint256(decimals())));
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        uint256 _amount = checkTrans(sender,recipient,amount);
        return TRC20.transferFrom(sender,recipient,_amount);
    }

    function transfer(address recipient, uint256 amount) public returns (bool) {
        uint256 _amount = checkTrans(msg.sender,recipient,amount);
        return TRC20.transfer(recipient,_amount);
    }

    function checkTrans(address _from ,address _to,uint256 amount) private returns (uint256) {
        if(whiteIn){
            require(_whitelist[_to] == 1, "TRC20: NOT WHITELIST IN");
        }
        if(whiteOut){
            require(_whitelist[_from] == 1, "TRC20: NOT WHITELIST OUT");
        }
        if(blackIn){
            require(_blacklist[_to] == 0, "TRC20: NOT BLACKLIST IN");
        }
        if(blackOut){
            require(_blacklist[_from] == 0, "TRC20: NOT BLACKLIST OUT");
        }

        return amount; 
    }

    function setCharge(address _address) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        charge = _address;
        return true;
    }

    function setAddr(address _address) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        fee_addr = _address;
        return true;
    }

    function setWhiteIn(bool _Is) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        whiteIn = _Is;
        return true;
    }

    function setWhiteOut(bool _Is) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        whiteOut = _Is;
        return true;
    }

    function setBlackIn(bool _Is) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        blackIn = _Is;
        return true;
    }

    function setBlackOut(bool _Is) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        blackOut = _Is;
        return true;
    }
    ...
    function setWhiteList(address _address) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        _whitelist[_address] = 1;
        return true;
    }

    function delWhiteList(address _address) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        _whitelist[_address] = 0;
        return true;
    }

    function setBlackList(address _address) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        _blacklist[_address] = 1;
        return true;
    }

    function delBlackList(address _address) public returns (bool) {
        require(msg.sender == owner, "TRC20: only owner");
        _blacklist[_address] = 0;
        return true;
    }
