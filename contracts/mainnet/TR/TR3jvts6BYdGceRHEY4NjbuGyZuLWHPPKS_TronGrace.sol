...
contract TronGrace {
	...
	uint constant public TRONGRACE_SPACE = 1000;
	...
	address payable public trongraceAddress;
    ...
    constructor(address payable marketingAddr, address payable projectAddr, address payable adminAddr, address payable networkAddr, address payable trongraceAddr) public {
        require(!isContract(marketingAddr) && !isContract(projectAddr));
        marketingAddress = marketingAddr;
        projectAddress = projectAddr;
        adminAddress = adminAddr;
		networkAddress = networkAddr;
		trongraceAddress = trongraceAddr;
    ...
    function invest(address referrer) public payable {
		...
		uint trongracespace = msgValue.mul(TRONGRACE_SPACE).div(PERCENTS_DIVIDER);

        marketingAddress.transfer(marketingFee);
        projectAddress.transfer(projectFee);
		adminAddress.transfer(adminFee);
		networkAddress.transfer(network);
		trongraceAddress.transfer(trongracespace);

        emit FeePayed(msg.sender, marketingFee.add(projectFee).add(network).add(trongracespace));
