...
/// @title
contract MultiSigLocker is BaseMultiSigWallet, UsddReceiver {
    using EnumerableSet for EnumerableSet.AddressSet;

    event StartUnlock(uint amount);
    event ExecuteUnlock(uint amount);
     ...
     *  Storage
     */
    address payable public preIssuer;

    uint public delay;
    uint public delayUnlockAmount;
    uint public unlockedAmount;

    mapping(uint => UnlockInfo) public unlockInfos;
     ...
     * Public functions
     */
    constructor (address[] memory _owners, uint _usdd, uint _mintDelay, address payable _preIssuer)
    ...
    validRequirement(_owners.length, 5)
    {
        ...
        delay = _mintDelay;
        preIssuer = _preIssuer;
    }

    function proposeUnlock(uint amount) public returns (uint transactionId) {
        require(getUsddBalance() > delayUnlockAmount && getUsddBalance() - delayUnlockAmount >= amount, "not enough usdd");

        uint txId = transactionCount;
        bytes memory data = abi.encodeWithSelector(this.startUnlock.selector, txId, amount);
        uint addedTxId = _submitTransaction(data);
        require(txId == addedTxId, "submit transactionId error");
        return addedTxId;
    }

    /// @dev Allows to send usdd to preIssuer. Transaction has to be sent by wallet.
    /// @param amount Address of new owner.
    function startUnlock(uint transactionId, uint amount)
    public
    onlyWallet
    {
        require(getUsddBalance() > delayUnlockAmount && getUsddBalance() - delayUnlockAmount >= amount, "not enough usdd");
        delayUnlockAmount += amount;

        UnlockInfo storage info = unlockInfos[transactionId];
        info.value = amount;
        info.timestamps = block.timestamp + delay;
        info.unlocked = false;

        emit StartUnlock(amount);
    }

    function executeUnlock(uint transactionId)
    public
    {
        UnlockInfo storage delayInfo = unlockInfos[transactionId];
        require(delayInfo.timestamps > 0, "unlock does not exist");
        require(delayInfo.timestamps <= block.timestamp, "unlock is not ready");
        require(!delayInfo.unlocked, "unlock is already finished");
        delayInfo.unlocked = true;

        uint amount = delayInfo.value;
        delayUnlockAmount -= amount;
        preIssuer.transferToken(amount, USDD);
        unlockedAmount += amount;

        emit ExecuteUnlock(amount);
    }
    ...
    function proposeReplaceOwner(address owner, address newOwner) public
    ownerExists(owner)
    ownerDoesNotExist(newOwner)
    returns (uint transactionId) {
    ...
    function getDelayInfos(uint _from, uint _to) public view returns (uint[] memory transactionIds, UnlockInfo[] memory delays) {
        require(_from < _to, "range not valid");
        require(_to <= transactionCount, "_to should be no larger than transactionCount");
        require(_to - _from <= 50, "length should be no larger than 50");

        uint cnt;
        uint txId;
        for (txId = _from; txId < _to; txId++) {
            if (unlockInfos[txId].timestamps > 0) {
                cnt ++;
            }
        }

        transactionIds = new uint[](cnt);
        delays = new UnlockInfo[](cnt);

        uint i;
        for (txId = _from; txId < _to; txId++) {
            if (unlockInfos[txId].timestamps > 0) {
                transactionIds[i] = txId;
                delays[i] = unlockInfos[txId];

                i++;
            }
        }

        return (transactionIds, delays);
    }
    ...
    returns (TransactionDetail memory detail) {
        ...
        if (selector == this.startUnlock.selector || selector == this.burn.selector) {
    ...
    function decodeCustomMethod(bool executed, bytes calldata data) public pure returns (TransactionDetail memory detail) {
        ...
        if (selector == this.startUnlock.selector) {
            (uint transactionId, uint amount) = abi.decode(params, (uint, uint));
            detail.method = "startUnlock(uint transactionId, uint amount)";
