...
//SourceUnit: LocalCoinSwapTrxEscrow.sol
...
contract LocalCoinSwapTrxEscrow {
  ...
  function createEscrow(
    ...
    require(_value > 2000000, "Escrow value too small");
    // Check transaction value against signed _value and make sure is not 0
    require(msg.value == _value && msg.value > 0, "Incorrect TRX sent");
  ...
  function doRelease(
    ...
    transferMinusFees(_buyer, _value, _txFee, _lcsFee);
  ...
  function resolveDispute(
    ...
    if (_winner == 1) {
      transferMinusFees(_seller, _value, _txFee, 0);
    }
  ...
  function sweepFees(uint256 _amount)
    external
    onlyWithdrawer
  {
    require(_amount <= collectedFees, "Amount is higher than amount available");
