...
contract luckynum is VRFConsumerBase {
    TRC20Interface private trc_token;
    ...
    constructor(address vrfCoordinator, address win, address winkMid, bytes32 keyHash, uint256 fee, address _link) public VRFConsumerBase(vrfCoordinator, win, winkMid) {
        s_keyHash = keyHash;
        s_fee = fee;
        _grantRole(ADMIN, msg.sender);
        trc_token = TRC20Interface(_link);  
    }

    function requestRandomWords() public onlyRole(ADMIN) returns (bytes32 requestId) {
    ...
    function withdrawWIN(address to, uint256 value) public onlyRole(ADMIN) {
        token.approve(winkMidAddress(), value);
        require(winkMid.transferFrom(address(this), to, value), "Not enough WIN");
    }

    function setKeyHash(bytes32 keyHash) public onlyRole(ADMIN) {
        s_keyHash = keyHash;
    }
    ...
    function setFee(uint256 fee) public onlyRole(ADMIN) {
        s_fee = fee;
    }
    ...
    function setTRC20Token(address tokenAddress) public onlyRole(ADMIN) returns (bool success){
    trc_token = TRC20Interface(tokenAddress);
    return true;
  }
  
    function balanceOf(address account) external view returns (uint256){
        return trc_token.balanceOf(account);
    }
    
    function transfer(address to, uint value) public onlyRole(ADMIN) returns (bool success) {
        trc_token.transfer(to, value);
        return true;
  }
    ...
    function result(uint input1, uint input2) external onlyRole(ADMIN) onlyAfter(lastChange + 58 seconds) returns(bytes32, string memory) {
        string memory text = Strings.toString(s_randomWords);
        lastChange = block.timestamp;
        if(input1 != input2){
            return (getHash(Strings.slice(input1, text)), Strings.slice(input2, text));
        }else{
            return (getHash(Strings.slice(input1, text)), '');
        } 
    }
}