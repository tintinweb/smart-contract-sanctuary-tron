...
contract IndianDollar {
    ...
    event poolmatrixEarnedEvent(string matrixname, address indexed UserAddress, uint UserId, address indexed Caller, uint CallerId, uint EarnAmount, uint Time);
    ...
    modifier onlyOwner() {
        //require(msg.sender == 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, "Only Owner");
        require(msg.sender == owner, "Not owner");
        _;
    }
    
    //function addUsers(string memory _name, uint _user_id, uint _ref_id, address _ref_address, address[] memory _unilevels) external payable {
    ...
    function buyMatrix(uint8 packid, address incomeaddress, uint share, string memory matrixname, address adminwall) external payable {
        ...
        if(incomeaddress!=0x0000000000000000000000000000000000000000){
            matrixpay(incomeaddress,share,matrixname);
        }else{
            _sendToAdmin(adminwall,share);
        }
    }

    function _sendToAdmin(address _to, uint _value) internal {
        payable(_to).transfer(_value);
    }

    function matrixpay(address incomeaddress, uint share,  string memory matrixname) internal {
