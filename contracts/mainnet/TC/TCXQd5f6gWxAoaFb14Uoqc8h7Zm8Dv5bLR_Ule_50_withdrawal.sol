...
contract Ule_50_withdrawal is Ownable{
  ...
  address public sub_owner;
       ...
       function changeTax(uint256 amount) external onlyOwner 
   {
    ...
    function claimAirdrop(uint256 _dolAmt) external {
        ...
        require(_dolAmt*1e18<=allowance[msg.sender],"Can't withdraw More than allowed!");
        require(_dolAmt*1e18<=maxDolAmt,"not claim more than max!");

        uint256 TRXVALUE = (_dolAmt*1e6)/2 ;
        uint256 TokenVALUE = (_dolAmt*1e18)/2 ;

        require(isBlackList[msg.sender]==false,"you are blocked!");
        require(claimPeriod==true,"Claim Periods Ends");
        
       
        uint256 taxValue = (tax.mul(TokenVALUE)).div(100);
        uint256 orignalValue = TokenVALUE.sub(taxValue);


        uint256 claimableToken =value(orignalValue);
        uint256 TrxValue = TRxvalue(TRXVALUE);
        require(claimableToken>=0,"You are not claimer");
        require(Token.balanceOf(address(this))>=claimableToken,"Contract is ran out of funds!");
        ...
        payable(msg.sender).transfer(TrxValue);
        lastClaim[msg.sender]=block.timestamp;
        allowance[msg.sender]=allowance[msg.sender].sub(_dolAmt*1e18);
    }
...
//........................................price.....................................................................
    ...
    // enter $ amount to get the ULE token
        function TRxvalue(uint256 _amt) public view returns(uint256)
    {
     return (_amt.mul(BnbtoBusd()).div(1e18));
    }
...
//   Ule     TLcKN2SBTAhiuUmkCvb86hRGdnV42cGyrt   18

//   USDT Lp  TFGDbUyP8xez44C76fin3bn3Ss6jugoUwJ  18
//   Ule Lp   TLdHJXL4QgJ7eYJuZs1qXqNuWerXq1Vhzs  18