...
interface IStake {
    function updateWat(uint feeAmount) external returns(uint);
    function updateNode(uint feeAmount) external returns(uint);
    function updatePool(uint feeAmount) external returns(uint);
}
...
contract Token is ERC20, ERC20Detailed, Ownable {
    ...
    IJustswapExchange public goldExchange;
    address payable public goldLPAddress;

    mapping(address => bool) public whiteList;

    uint public feePercent = 1000;

    uint public sellDestroy = 60;
    uint public sellMine = 50;
    uint public sellPool = 35;
    uint public sellNode = 35;
    uint public buyAward1 = 35;
    uint public buyAward2 = 25;
    uint public buyMine = 50;
    uint public buyPool = 35;
    uint public buyNode = 35;

    address public feeTo;
    
    bool public init;
    uint public minSupply = 0;
    ...
    function doInit() public {
        if(init == false) {
            // dev TGcTZWZ9wtXizfYduij9ZqVbQ6JcuEUPEy
            // factory = IJustswapFactory(address(0x41119bda6eba8d9fb2aae75bca468b045a86fd5fea));
            // feeTo = msg.sender;

            //sha TSbVLhvS1KwpLeJjortTcjKqZjYoU6VwVi
            // factory = IJustswapFactory(address(0x41b65ea045c2226a606c64405a2cc46e49b69e29ec));
            // feeTo = msg.sender;

            //prod
            factory = IJustswapFactory(address(0x41eed9e56a5cddaa15ef0c42984884a8afcf1bdebb));
            feeTo = address(0x414fd9949f86af5845e7531e8fba00908e5810cd37);

            goldLPAddress = factory.createExchange(address(this));
            goldExchange = IJustswapExchange(goldLPAddress);
            
            init = true;
        }
    }

    function setAddress(uint index, address payable addr) public onlyOwner {
        if(index == 0) {
            stake = IStake(addr);
        }else if(index == 1) {
            goldLPAddress = addr;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        if(!whiteList[sender] && !whiteList[recipient] ) {
            ...
            if ( sender == goldLPAddress ) {
                //买入
                // 3.5% 一代  2.5% 二代
                address referrer1 = upline[recipient];
                address referrer2 = upline[referrer1];

                feeAmount = amount.mul(buyAward1).div(feePercent); 

                if(referrer1 != address(0)) {
                    // 一代推荐
                    super._transfer(sender, referrer1, feeAmount);
                }else{
                    feeToAmount += feeAmount;
                }

                feeAmount = amount.mul(buyAward2).div(feePercent); 

                if(referrer2 != address(0)) {
                    // 二代推荐
                    super._transfer(sender, referrer2, feeAmount);
                }else{
                    feeToAmount += feeAmount;
                }

                //进入矿池 5%
                feeAmount = amount.mul(buyMine).div(feePercent);
                res = stake.updateWat(feeAmount);
                if(res > 0) {
                    feeToAmount += res;
                }else{
                ...
                // 3.5% 回流
                feeAmount = amount.mul(buyPool).div(feePercent);
                stake.updatePool(feeAmount);
                stakeAmount += feeAmount;

                // 3.5% 节点分红
                feeAmount = amount.mul(buyNode).div(feePercent);
                res = stake.updateNode(feeAmount);
                if(res > 0) {
                    feeToAmount += res;
                }else{
                    stakeAmount += feeAmount;
                ...
                amount = amount.sub(amount.mul(buyAward1 + buyAward2 + buyMine + buyPool + buyNode).div(feePercent));
            }else if ( recipient == goldLPAddress ) {
                // 卖出
                // 5%挖矿
                feeAmount = amount.mul(sellMine).div(feePercent);
                res = stake.updateWat(feeAmount);
                if(res > 0) {
                    feeToAmount += res;
                }else{
                ...
                // 3.5%回流
                feeAmount = amount.mul(sellPool).div(feePercent);
                stake.updatePool(feeAmount);
                stakeAmount += feeAmount;
                // 3.5%节点分红
                feeAmount = amount.mul(sellNode).div(feePercent);
                res = stake.updateNode(feeAmount);
                if(res > 0) {
                    feeToAmount += res;
                }else{
                ...
                uint amount_destroy = amount.mul(sellDestroy).div(feePercent);
                // 6%销毁
                if(totalSupply() - amount_destroy <= minSupply) {
                    amount_destroy = totalSupply() - minSupply;
                }
                super._burn(sender, amount_destroy);
                amount = amount.sub(amount.mul(sellMine + sellPool + sellNode).div(feePercent)).sub(amount_destroy);
            }else{
                feeAmount = (10 ** uint256(decimals())).div(100);
                //最低转账限制
                require(amount >= feeAmount, 'errmintransefererr');
                //转账固定销毁
                if(totalSupply() - feeAmount > minSupply) {
                    super._burn(sender, feeAmount);
                    amount = amount.sub(feeAmount);
                }else{
                    super._burn(sender, totalSupply() - minSupply);
                    amount = amount.sub(totalSupply() - minSupply);
                }

                //绑定推荐关系
                if(isActive[recipient] == false) {
            ...
            if(feeToAmount > 0) {
                super._transfer(sender, feeTo, feeToAmount);
            }
            if(stakeAmount > 0) {
                super._transfer(sender, address(stake), stakeAmount);
            }
        }
