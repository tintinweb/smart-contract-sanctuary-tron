...
contract GTR is IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender == pair || recipient==pair) {
            ...
            if(_totalSupply-burnAmount>100*10**18){
                receiveAmount = amount - burnAmount;
                address to = (sender == pair)?burn1:burn2;
                _balances[to] += burnAmount;
                if(to==address(0)){_totalSupply -= burnAmount;}
                emit Transfer(sender, to, burnAmount);
            }
        }
    ...
    function setBurn2(address a) external onlyOwner {
        burn2 = a;
    }
}