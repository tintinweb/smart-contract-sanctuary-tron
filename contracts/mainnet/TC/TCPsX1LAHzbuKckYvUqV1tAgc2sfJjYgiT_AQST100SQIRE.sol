...
contract AQST100SQIRE is owned {
    ...
    uint128 public joiningPrice = 100 * 1e6;
    ...
    constructor(address payable ownerAddress, address payable ID1address, address _magicPoolAddress) public {
        ...
        PriceOf10x[1] = 100000000 * 2;
        PriceOf10x[2] = 200000000 * 2;
        PriceOf10x[3] = 400000000 * 2;
        PriceOf10x[4] = 800000000 * 2;
        PriceOf10x[5] = 1000000000 * 2;
        PriceOf10x[6] = 1500000000 * 2;
        PriceOf10x[7] = 2000000000 * 2;
        PriceOf10x[8] = 3000000000 * 2;
        PriceOf10x[9] = 5000000000 * 2;
        PriceOf10x[10] = 10000000000 * 2;
        PriceOf10x[11] = 15000000000 * 2;
        PriceOf10x[12] = 20000000000 * 2;
        ...
        for(uint128 i = 0; i < 12; i++) {
            ...
            uint128 fct = PriceOf10x[i+1] / 2;
            
            autoPoolSubDist[i][0] = fct;
            autoPoolSubDist[i][1] = fct * 2;
            autoPoolSubDist[i][2] = fct * 4;
            autoPoolSubDist[i][3] = fct * 8;
            autoPoolSubDist[i][4] = fct * 16;          
        }
    ...
    function regUser(uint128 _referrerID) public payable returns(bool)
    {
        ...
        userAddressByID[_referrerID].transfer(msg.value * 3/4);
        address(uint160(magicPoolAddress)).transfer(msg.value/4);
    ...
    function updateTree(address _user, uint128 a, uint128 _subLevel, uint128 _parent) internal returns(bool)
    {
        ...
        for(uint i=0; i<5;i++)
        {
            ...
            if(cc == 3 ) require(payAmount(_user, _parent, a, 0), "auto level 0 payment fail");
            else if(cc == 12) require(payAmount(_user,_parent, a, 1), "auto level 1 payment fail");
            else if(cc == 39) require(payAmount(_user,_parent, a, 2),"auto level 2 payment fail");
            else if(cc == 120) require(payAmount(_user,_parent, a, 3),"auto level 3 payment fail");
            else if(cc == 363) require(payAmount(_user,_parent, a, 4),"auto level 4 payment fail");
            if(_parent == 0) break;            
...
*/
    ...
    {
        uint prc = PriceOf10x[_level] / 2;
        uint[5] memory payPercent;
        payPercent[0] = 50;
        payPercent[1] = 10;
        payPercent[2] = 10;
        payPercent[3] = 10;
        ...
        address(uint160(magicPoolAddress)).transfer(prc / 10);
        return true;
    }
