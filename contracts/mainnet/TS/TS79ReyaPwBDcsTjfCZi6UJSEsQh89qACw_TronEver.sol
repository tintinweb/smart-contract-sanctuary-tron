//SourceUnit: Tronever.sol

/*                                   
 *                                                                  
 * TronEver Smart Contract Made Actually Smart
 * Verified Smart Contract Investment Platform.
 * Audited by JSL SOLUTIONS - Independat Smart Contract Validation Company
 * TronEver.com is a Tron blockchain smart contract-based investment platform founded by professionals behind the biggest mining company globally- CoinCrafts.com. we’ve leveraged on blockchain technology as well as established our code audit run by an independent audit company. Our user’s funds are kept safe away from any third-party control for instance, no company, institution, or individual can access the funds, even the TronEver administration team.
 ...
 *                                                                                                                  
 *                                                                                                                   
 * Total Platforms Profit for the Investment period is 160%, which Guarantees LONG and STABLE Income.
 * Hold Bonus is limited to 1% Total and it gives extra 0.1% For Every Day you hold your Balance without Withdrawal.
 * Total Daily Profit Is Limited to 3.6%, from which 1% Is Total Hold Bonus and 3% Daily Profit. Contract starts with 2% Daily.
 * Daily Profit Increases for 0.1% every time Daily Profit Increases for 0.1% when Smart Contract Balance Reaches + 1 Million TRX. And the decreases when the balance gets lower.
 ...
 * 
 *
 *   - 75% Main Smart Contract Balance for Payouts
...
contract TronEver {
	...
	uint256 constant public MarketingFee = 1100;
	uint256 constant public ServiceFee = 400;
	uint256[] public ReferralCommissions = [500, 300, 200];
	uint256 constant public Day = 1 days;
	uint256 constant public ROICap = 16000;
	uint256 constant public PercentDiv = 10000;
	uint256 constant public ContractIncreaseEach = 1000000 trx;
	uint256 constant public StartBonus = 160;
			...
			if (user.deposits[i].withdrawn < ((user.deposits[i].amount.mul(ROICap)).div(PercentDiv))) {
				if (user.deposits[i].start > user.checkpoint) {
					dividends = (user.deposits[i].amount.mul(userPercentRate).div(PercentDiv))
						...
						.div(Day);
					ResetHoldBonus = ResetHoldBonus.add(10);
				} else {
					dividends = (user.deposits[i].amount.mul(userPercentRate).div(PercentDiv))
