...
contract TronArmy is Ownable{
    
 	using SafeMath for uint256;
    
    address payable private adminAccount_;
    
    event Registration(string  waddress,address investor,uint256 referrerId,uint256 package,string clubname);
    ...
    function UserRegister( string memory _user,uint256 _referrerCode,uint256 package,string memory clubname) public payable
    {  

        require(msg.value==package,"Invalid Amount");
	  // payable(owner()).transfer(address(this).balance);
	    adminAccount_.transfer(address(this).balance);
        emit Registration(_user,msg.sender,_referrerCode,msg.value,clubname);
    }

    function reinvest(address _user,string memory investorId,uint256 package,string memory plan_name) public payable
    {
        require(msg.value==package,"Invalid Amount");
		//payable(owner()).transfer(address(this).balance);
		 adminAccount_.transfer(address(this).balance);
  	    emit Deposit(msg.sender,investorId,msg.value,plan_name);
    }
