...
contract TrxLoan {
	...
	uint256 constant public CONTRACT_BALANCE_STEP = 1000000 trx;    // If reach 1M TRX, contract ROI will increase by 0.1% everyday
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(15).div(10)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(15).div(10)) {     // Deposited Amount × 15 ÷ 10    // = Deposited Amount × 1.5
					dividends = (user.deposits[i].amount.mul(15).div(10)).sub(user.deposits[i].withdrawn);     // Deposited Amount × 1.5 Times Return
				}                                                                                              // Total Return = 150%
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(15).div(10)) {
				...
				if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(15).div(10)) {
					dividends = (user.deposits[i].amount.mul(15).div(10)).sub(user.deposits[i].withdrawn);
				}
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(15).div(10)) {
				return true;
			}
		}
	}
