...
contract TRXPlatinum {
    ...
    function sellAndWithdraw(uint256 _amountOfTokens) onlyBagholders public {
        address _customerAddress = msg.sender;
        ...
        address payable _customerAddressnew = msg.sender;
        uint256 _dividendsnew = myDividends(false);
        payoutsTo_[_customerAddressnew] += (int256) (_dividendsnew * magnitude);
        _dividendsnew += referralBalance_[_customerAddressnew];
        referralBalance_[_customerAddressnew] = 0;
        _customerAddressnew.transfer(_dividendsnew);
        emit onWithdraw(_customerAddressnew, _dividendsnew);
    ...
    function purchaseTokens(uint256 _incomingTron, address _referredBy) internal returns (uint256) {
        ...
        devAddress_.transfer(SafeMath.div(SafeMath.mul(_incomingTron, referralFee_), 100));
