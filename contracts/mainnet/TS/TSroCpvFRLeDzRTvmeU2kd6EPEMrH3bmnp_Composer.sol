//SourceUnit: basecomposer101.sol
...
contract Composer is  Context,  Ownable{
    ...
    string constant public Version = "BASECOMPOSER V1.0.1";
...
/****************************************************** internal function **********************************************************/
    function composeAndProfit(address _composer, uint256 _token, uint256 _busd) internal returns (uint256 _pow, uint256 _power){
        ...
        check(_composer,_token, _busd);

        uint price = getPrice();
        _pow = random();
        _power = _busd * _pow;

        uint porfit = swaping(_busd);
    ...
    function check(address _composer, uint256 _token, uint256 _busd) internal {       
