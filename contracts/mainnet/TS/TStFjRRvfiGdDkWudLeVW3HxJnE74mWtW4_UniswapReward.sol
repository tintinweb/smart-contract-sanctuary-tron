...
contract LPTokenWrapper is IPool,Governance {
    ...
    function setnft_token(address one_token) public  onlyGovernance {
        //return _totalSupply;
        _nftToken = one_token;
    }
    ...
    function stake(uint256[] memory tokenids) public {
        ...
        _totalSupplyone = _totalSupplyone.add(tokenids.length);
        _balancesOne[msg.sender] = _balancesOne[msg.sender].add(tokenids.length);

    }


    function withdraw(uint256 amount) public {
        require(amount >= _balancesOne[msg.sender], "amout >= balances");
        uint idlen = _balancesOne[msg.sender].sub(1);

         _totalSupplyone = _totalSupplyone.sub(amount);
         _balancesOne[msg.sender] = _balancesOne[msg.sender].sub(amount);
...
contract UniswapReward is LPTokenWrapper{
    ...
    uint256 public _initReward = 1000 * 1e18;
    ...
    constructor(address _newstaketoken, address _newreardtooken) public {
        _nftToken = _newstaketoken;
        _rewardToken = IERC20(_newreardtooken);
    }
