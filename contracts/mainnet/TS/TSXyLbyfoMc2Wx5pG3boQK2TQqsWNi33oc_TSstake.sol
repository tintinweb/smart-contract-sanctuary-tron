...
interface Imetedao {
  function setbindParent(address account, address user) external;
}
...
contract TSstake is LPTokenWrapper, Ownable {
    IERC20 public token;
    ...
    Imetedao public LP2 = Imetedao(address(0));
    ...
    function setLP2(address _LP2) public onlyOwner {
      LP2 = Imetedao(_LP2);
    }
    ...
    function bindParent(address parent) public returns (bool) {
        ...
        require(_parents[parent] != address(0) || parent == deployer, 'no parent');
        _parents[msg.sender] = parent;
        _mychilders[parent].push(msg.sender);

        LP2.setbindParent(parent, msg.sender);
