...
contract Ncomp is ERC721Holder,Ownable{
    ...
    IJustswapExchange public mineExchange;
    IJustswapExchange public usdtExchange;

    uint public destoryGain = 2439e4;

    mapping(uint => bool) public mayDestoryLevelMap;

    uint public convertBaseGain = 813e5;
    mapping(uint => uint) public convertTimes;

    uint public convertNftTypeProduct = 479001600;
    uint public convertNftTypeSum = 78;

    event Convert(address indexed user,uint time,uint amount);
    event Destory(address indexed user,uint time,uint tokenId,uint amount);
    event Comp(address indexed user,uint time,uint tokenId);


    constructor(
        address _nft,
        address _mineToken,
        address _mineExchange,
        ...
        mineExchange = IJustswapExchange(_mineExchange);
        usdtExchange = IJustswapExchange(_usdtExchange);

        mayDestoryLevelMap[1] = true;
        mayDestoryLevelMap[2] = true;
        convertTimes[1] = 2;
        convertTimes[2] = 2;
        convertTimes[3] = 2;
        convertTimes[4] = 2;
    ...
    function comp(uint[] memory tokenIds,bytes32 flag)external {

        uint len = tokenIds.length;

        require(len == 3,"tokenIds length error");

        uint typesSum = 0;
        uint levelSum = 0;
        for( uint i = 0; i < len; i++ ){
            (uint types,uint level,) = nft.getTokenInfo(tokenIds[i]);

            typesSum += types;
            levelSum += level;

            require( typesSum / types == i + 1 && levelSum / level == i + 1,"types or level diff");
        }

        uint t = typesSum / len;
        uint l = levelSum / len;

        require(l < nft.getMaxLevel(),"max level");

        nft.safeTransferFromBatch(msg.sender, address(0xdead), tokenIds);

        uint id = nft.mint(msg.sender, uint8(t), uint8(l + 1));

        resultMap[flag] = id;

        emit Comp(msg.sender, block.timestamp, id);
    }
    ...
    function _worthConvert(uint usdtAmount)internal view returns(uint){
        ...
        return IJustswapExchange(mineExchange).getTrxToTokenInputPrice(trxAmount);
    }

    function destory(uint tokenId) external{
        require( !_isContract(msg.sender),"not allow");
        (,uint level,) = nft.getTokenInfo(tokenId);

        require(mayDestoryLevelMap[level],"not may destory");

        nft.safeTransferFrom(msg.sender, address(0xdead), tokenId);

        uint tokenAmount = _worthConvert(destoryGain);

        if(tokenAmount > 0 ){
            mineToken.transfer(msg.sender, tokenAmount);
            emit Destory(msg.sender,block.timestamp,tokenId,tokenAmount);
        }
    }

    function getDestoryGain(uint tokenId)external view returns(uint){
        (,uint level,) = nft.getTokenInfo(tokenId);

        if(!mayDestoryLevelMap[level]) return 0;

        return _worthConvert(destoryGain);
    }

    function convert(uint[] memory tokenIds)external{
        require( !_isContract(msg.sender),"not allow");
        require(tokenIds.length == 12,"length error");

        (uint product,uint sum,uint level) = nft.getTokenInfos(tokenIds);

        require( product == convertNftTypeProduct && sum == convertNftTypeSum,"nft type error");

        nft.safeTransferFromBatch(msg.sender, address(0xdead), tokenIds);

        uint usdtAmount = convertTimes[level] * convertBaseGain;

        if( usdtAmount > 0 ){
            uint tokenAmount = _worthConvert(usdtAmount);
            mineToken.transfer(msg.sender, tokenAmount);
            emit Convert(msg.sender,block.timestamp,tokenAmount);
        }
    }

    function getConvert(uint[] memory tokenIds)external view returns(uint){
        ...
        return _worthConvert(convertTimes[level] * convertBaseGain);
    }
    ...
    function SetDestoryGain(uint amount)external onlyOwner{
        destoryGain = amount;
    }

    function SetConvertBaseGain(uint amount)external onlyOwner{
        convertBaseGain = amount;
    }

    function SetConvertTimes(uint level,uint times)external onlyOwner{
        convertTimes[level] = times;
    }

    function SwitchMayDestoryLevelMap(uint level,bool boolV)external onlyOwner{
        mayDestoryLevelMap[level] = boolV;
    }
}