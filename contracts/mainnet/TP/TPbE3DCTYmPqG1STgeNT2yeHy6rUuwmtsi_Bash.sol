...
contract BASH {
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        uint256 _allaff = (_amount*1728)/(100000);
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            if(up == address(0)) break;
            
            uint256 bonus = _amount * ref_bonuses[i] / 100;
            _allaff=_allaff-bonus;
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        require(player.deposits.length < 50, "Max 50 deposits per address");
    ...
    */
    ...
    function addTarif(uint16 _days, uint16 _percent) ownerOnly public{
        tarifs.push(Tarif(_days, _percent));
