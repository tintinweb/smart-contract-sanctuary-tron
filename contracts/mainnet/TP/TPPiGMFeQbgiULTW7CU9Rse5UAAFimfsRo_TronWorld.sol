 ...
 *   - Contract total amount bonus: +0.2% for every 1.000.000,00 TRX on platform address balance
 *
 ...
 *   [AFFILIATE PROGRAM]
 *
 *   - 3-level referral commission: 5% - 1% - 1%
 *
 *   --------------------------------------------------------------------------------------------------------
 *
 *   [FUNDS DISTRIBUTION]
...
contract TronWorld{
	...
	uint256[] public REFERRAL_PERCENTS = [50, 10, 10]; // 5%, 1%, 1%
  ...
  function getUserHoldPercent(address userAddress) external view returns (uint256) {
	...
	function getContractBalanceRate() public view returns (uint256) {
		...
		return BASE_PERCENT.add(contractBalancePercent * 2); //0.2% every 1m
	}
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			uint256 timeMultiplier = (block.timestamp.sub(user.checkpoint)).div(TIME_STEP);
			return contractBalanceRate.add(timeMultiplier * 3); //0.5%
		} else {
