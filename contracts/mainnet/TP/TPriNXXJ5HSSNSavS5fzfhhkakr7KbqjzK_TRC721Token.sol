...
contract TRC721Metadata is Context, TRC165, TRC721, ITRC721Metadata {
    ...
    // If all tokens the same uri
    bool private _sameUri;
    ...
    function _flipSameURI() internal {
        _sameUri = !_sameUri;
    }
...
contract TRC721Token is TRC721, TRC721Enumerable, TRC721Metadata, MinterRole {

    uint256 public MAX_SUPPLY = 30;

    constructor() public TRC721Metadata("Trailblazer", "MTT") {
    }
    ...
    function flipSameURI() public onlyMinter {
        _flipSameURI();
    }

    /**
     * @dev Function to batch mint tokens.
     * @param _owners The list of addresses that will receive the minted token.
     * @return A boolean that indicates if the operation was successful.
     */
    function batchMint(address[] memory _owners) public onlyMinter returns (bool) {
        require(totalSupply().add(_owners.length) <= MAX_SUPPLY, "This operation would exceed the maximum supply");
    ...
    function mint(address to) public onlyMinter returns (bool) {
        require(totalSupply() < MAX_SUPPLY, "This operation would exceed the maximum supply");
        _mint(to, totalSupply());
        return true;
    }
