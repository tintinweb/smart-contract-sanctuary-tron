 ...
 *
 *  - Developer &  project manager  : Dr.BlockChain  - TEAM : R.Kh & A
 *    
 *	----------------------------*** ENJOY IT ***-----------------------------
***/
...
contract TronTower2 is Pausable{
    ...
    uint256 constant public PAY_PERCENT = 5;  
    ...
    event Deposited(address indexed addr, address indexed referrer, uint256 amount, uint256 box);
	...
	function setLOTTERYTEAM_PERCENT (uint _LOTTERY_PERCENT , uint _TEAM_PERCENT) public onlyOwner {
    ...
    function deposit(address referrer, uint256 boxId) public payable whenNotPaused {
        ...
        emit Deposited(addr, referrer, amount, boxId);
    }

    function reinvest(uint256 amount, uint256 boxId ) public whenNotPaused {
        require(boxId < 3, "The box must be chosen correctly!");
        ...
        emit Withdrawn(GUARANTEED_WALLET, guaranteedAmount);
        emit Withdrawn(TEAM_WALLET, adminFee);
        emit Deposited(addr, users[addr].parent, amount, boxId);
    }
