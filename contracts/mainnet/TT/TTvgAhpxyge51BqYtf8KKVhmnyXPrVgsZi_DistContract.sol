...
contract DistContract is  Context,  Ownable{
    ...
    string constant public Version = "BASEDIST V1.0.0";
    ...
    function MintAndDist(uint256 _amount) public onlyOwner {
        ...
        IBEP20(token).transfer(reward, _amount);
        address[] memory path = new address[](2);
        path[1] = busd; path[0] = token;
        uint[] memory _price = uniswapV2Router.getAmountsOut(1e18, path);
        IReward(reward).RewardProfit(_amount, _price[1]);
    }

    function Dist(uint256 _amount) public returns (bool){
        IBEP20(token).transferFrom(_msgSender(), reward, _amount);
        IBEP20(token).approve(reward, _amount);
        address[] memory path = new address[](2);
        path[1] = busd; path[0] = token;
        uint[] memory _price = uniswapV2Router.getAmountsOut(1e18, path);
        IReward(reward).RewardProfit(_amount, _price[1]);
