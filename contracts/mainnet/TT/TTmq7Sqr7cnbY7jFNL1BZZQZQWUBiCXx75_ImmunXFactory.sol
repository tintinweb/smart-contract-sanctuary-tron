...
contract ImmunXFactory {
    ...
    address private adminLevelFive;

    uint private gasTrx = 0;
    ...
    modifier _onlyOwner(address _address) {
        require(
            msg.sender == _address,
    ...
    bool private adminSet = false;

    function setAdmins(
        address payable _adminOne,
        address _adminTwo,
        address _adminThree,
        address _adminFour,
        address _adminFive
    ) public _onlyOwner(superAdmin) {
        require(adminSet == false, "Admins are only set once!");

        adminLevelOne = _adminOne;
        adminLevelTwo = _adminTwo;
        adminLevelThree = _adminThree;
        adminLevelFour = _adminFour;
        adminLevelFive = _adminFive;
    ...
    function _adminFee(uint _investmentAmount) internal {
        uint _totalAdminFeeToDeduct = _investmentAmount.mul(50).div(100);

        usdtAddress.transfer(adminLevelOne, _totalAdminFeeToDeduct.mul(18).div(100));
        usdtAddress.transfer(adminLevelTwo, _totalAdminFeeToDeduct.mul(18).div(100));
        usdtAddress.transfer(adminLevelThree, _totalAdminFeeToDeduct.mul(16).div(100));
        usdtAddress.transfer(adminLevelFour, _totalAdminFeeToDeduct.mul(16).div(100));
        usdtAddress.transfer(adminLevelFive, _totalAdminFeeToDeduct.mul(16).div(100));
        usdtAddress.transfer(superAdmin, _totalAdminFeeToDeduct.mul(16).div(100));
    }
    ...
    //-------------------------------------------------------------------------------------------------------
    function _setUser(uint _investmentAmount) internal {
        User storage user = users[msg.sender];

        user.initialInvestment = _investmentAmount;

        /* @dev User can only withdraw the ROI after 7 day of Investment */
        user.ROIClaimableAt = block.timestamp.add(86400 * 25);
        user.withdrawableDisablesAt = block.timestamp.add(86400 * 100);
    ...
    function investmentWithoutReferral(uint _investmentAmount) public  _minInvestment(_investmentAmount) {
        _setUser(_investmentAmount);
        ...
        usdtAddress.transfer(adminLevelOne, _investmentAmount.mul(18).div(100));
    }
    ...
    function _referralFee(uint _investmentAmount) private {
        ...
        if (referralLevelCount[msg.sender][3] != false) {
            usdtAddress.transfer(referralLevel[msg.sender][1], _investmentAmount.mul(3).div(100));
            usdtAddress.transfer(referralLevel[msg.sender][2], _investmentAmount.mul(6).div(100));
            usdtAddress.transfer(referralLevel[msg.sender][3], _investmentAmount.mul(9).div(100));

            totalReferralBonus += _investmentAmount.mul(18).div(100);
        } else {
            if (referralLevelCount[msg.sender][2] != false) {
                usdtAddress.transfer(referralLevel[msg.sender][1], _investmentAmount.mul(6).div(100));
                usdtAddress.transfer(referralLevel[msg.sender][2], _investmentAmount.mul(9).div(100));
    ...
    function withdrawAndReinvest() public {
        require(
            users[msg.sender].ROIClaimableAt < block.timestamp,
            "Dividend withdrawing request before time!"
        );

        uint withdrawable = 50000000 * 70;
        uint reinvested = 50000000 * 30;
    ...
    //-------------------------------------------------------------------------------------------------------
    function reInvest(uint _investmentAmount) public _minInvestment(_investmentAmount) {        
        users[msg.sender].initialInvestment = _investmentAmount;

        /* @dev User can only withdraw te ROI after 7 day of Investment */
        users[msg.sender].ROIClaimableAt = block.timestamp.add(86400 * 25);
        users[msg.sender].withdrawableDisablesAt = block.timestamp.add(86400 * 100);
