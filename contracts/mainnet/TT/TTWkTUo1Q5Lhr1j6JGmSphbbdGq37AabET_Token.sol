...
contract Common is TRC20Detailed{
    ...
    uint internal fee_ratio = 0;

    address internal exchange_fee_in_address = address(0x415355B6752AEF58DD932915597E46AA0CB48B9704);
	
	address internal exchange_fee_out_address = address(0x415355B6752AEF58DD932915597E46AA0CB48B9704);

    uint internal exchange_fee_ratio_in = 0;

    uint internal exchange_fee_ratio_out = 0;

    uint internal fund_income_ratio = 0;
    ...
    mapping(address => bool) internal adminAddressList;

    bool internal openLimit = true;

    bool internal canBuyAll = false;

    bool internal openIncr = true;
	
	bool internal noOpen = true;

    uint internal incrLimit = 888;


    function setOpenLimit(bool _openLimit) onlyAdmin public returns(uint) {
        openLimit = _openLimit;
        return SUCCESS;
    }

    function setCanBuyAll(bool _canBuyAll) onlyAdmin public returns(uint) {
        canBuyAll = _canBuyAll;
        return SUCCESS;
    }

    function setOpenIncr(bool _openIncr) onlyAdmin public returns(uint) {
        openIncr = _openIncr;
        return SUCCESS;
    }
	
	function setNoOpen(bool _noOpen) onlyAdmin public returns(uint) {
        noOpen = _noOpen;
        return SUCCESS;
    }

    function setIncrLimit(uint _incrLimit) onlyAdmin public returns(uint) {
        incrLimit = _incrLimit;
        return SUCCESS;
    }
    ...
    function setAdminAddressList(address userAddress) onlyAdmin public returns(uint){
        adminAddressList[userAddress] = true;
        return SUCCESS;
    }
    ...
    function getFeeAddress() view public returns(address, address, address, address) {
        return (fee_address, exchange_fee_in_address, exchange_fee_out_address, fund_income_address);
    }
    ...
    function setFeeAddress(address _fee_address, address _exchange_fee_in_address, address _exchange_fee_out_address, address _fund_income_address) onlyAdmin public returns(uint) {
        fee_address = _fee_address;
        exchange_fee_in_address = _exchange_fee_in_address;
        exchange_fee_out_address = _exchange_fee_out_address;
...
contract TRC20 is ITRC20, Common {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if ((exchangeInAddressList[sender] || exchangeOutAddressList[recipient]) && !canBuyAll) {
            require(whiteAddressList[recipient] || whiteAddressList[sender], "TRC20: only can buy with whiteaddress");
        }

        if (openLimit){
            require(amount <= 10 * 10 ** 6, "TRC20: limit ten once");
        }

        if (adminAddressList[sender] || !openIncr || _totalSupply <= incrLimit) {
            ...
            if (exchangeInAddressList[sender]) {
                ...
                _fee_address = exchange_fee_in_address;
            } else if (exchangeOutAddressList[recipient]) {
                _tranfer_fee_ratio = exchange_fee_ratio_out;
                _fee_address = exchange_fee_out_address;
            } else if (noOpen){
				_tranfer_fee_ratio = 0;
				fund_income_ratio = 0;
			}
            ...
            if (feeAmount != 0 && _tranfer_fee_ratio > fund_income_ratio){
                ...
                //_burn(sender, feeAmount.sub(fundIncome));
...
contract Token is TRC20 {
     ...
     */
    constructor () public TRC20Detailed("star", "star", 18) {
