//SourceUnit: BSGV2.sol

/*
 *   BSGV2 - Blockchain Split Game Version 2.0!
 *
 *   ┌───────────────────────────────────────────────────────────────────────┐
 *   │   PC Web: https://bsgv2.com                                           │
 *   │   Mobile Web: https://m.bsgv2.com                                     │
 *   │                                                                       │
 *   │   Telegram Public Group: https://t.me/bsgv2                           |
 *   |                                                                       |
 *   |   E-mail: bsgv2@proton.me                                             |
 *   └───────────────────────────────────────────────────────────────────────┘ 
 *
 *   [Platfrom Rules]
 * 
 *   - 15 days per cycle, earnings 2% daily, 30% per cycle, 60% per month
 *   - 20-tier invitation system
 *   - Three different participation rewards
 * 
 *   - Minimal deposit: 50 USDT
 *   - Maximal deposit: 2000 USDT
 *   - Unlimited times for participation
 * 
 *   Share your referral link with your partners and get additional bonuses.
 *   - 20-level referral commission: 
 *     - for 1 star users: 5%
 *     - for 4 star users: 1 star + 1% - 2% - 3% - 1%
 *     - for 5 star users: 1 star + 4 start + 2% - 1% - 1% - 1% - 1% - 0.5% * [11 - 20]
 *
 *   [Fund Distribution]
 *
 *   - 98% Balance for distribution to participants
 *   - 1% DevOps fee
 *   - 1% Market operation fee
 *
 */
...
contract BSGV2 {
    ...
    uint256 private constant freezeIncomePercents = 4000;
    ...
    uint256 private constant maxStarUser = 100;
    ...
    event UpdateLevel(address user, uint256 level);

    constructor(address _usdtAddr, address _defaultRefer, address[2] memory _feeReceivers, uint256 _startTime) public {
    ...
    function updateLevel(address _user) public {
        ...
        uint256 levelNow = calLevelNow(_user);
        ...
        emit UpdateLevel(_user, levelNow);
    }
    ...
    function getMaxFreezing(address _user) public view returns(uint256) {
        ...
        for(uint256 i = orderInfos[_user].length; i > 0; i--){
            OrderInfo memory order = orderInfos[_user][i - 1];
    ...
    function calLevelNow(address _user) public view returns(uint256) {
        UserInfo memory user = userInfo[_user];
        uint256 max = user.maxDeposit;
        uint256 levelNow;
        if(max >= 1000e6){
            (uint256 maxTeam, uint256 otherTeam, ) = getTeamDeposit(_user);
            if(max >= 2000e6 && user.teamNum >= 200 && maxTeam >= 50000e6 && otherTeam >= 50000e6){
                levelNow = 5;
    ...
    function _distributeStarPool() private {
        uint256 level4Count;
        for(uint256 i = level4Users.length; i > 0; i--){
            if(userInfo[level4Users[i - 1]].level == 4){
                level4Count = level4Count.add(1);
                if(level4Count >= maxStarUser) break;
            }
        }
        ...
        if(level4Count > 0){
            ...
            uint256 totalReward;
            for(uint256 j = level4Users.length; j > 0; j--){
                if(userInfo[level4Users[j - 1]].level == 4){
                    rewardInfo[level4Users[j - 1]].star = rewardInfo[level4Users[j - 1]].star.add(reward);
                    userInfo[level4Users[j - 1]].totalRevenue = userInfo[level4Users[j - 1]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                    level4Count = level4Count.sub(1);
                    if(level4Count == 0) break;
                }
            }
    ...
    function _distributeLuckPool(uint256 _dayNow) private {
        ...
        if(dayDepositCount > 0){
            ...
            for(uint256 j = dayDepositCount; j > dayDepositCount.sub(checkCount); j--){
                address userAddr = dayLuckUsers[_dayNow - 1][j - 1];
                if(userAddr != address(0)){
                    uint256 reward = luckPool.mul(dayLuckUsersDeposit[_dayNow - 1][j - 1]).div(totalDeposit);
                    totalReward = totalReward.add(reward);
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                if((i > 0 && i < 5 && userInfo[upline].level > 3) || upline == defaultRefer){
