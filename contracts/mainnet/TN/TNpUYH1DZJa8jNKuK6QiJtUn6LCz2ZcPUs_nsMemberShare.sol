...
contract Common {
    ...
    // ns合约地址
    address constant ns_contract = address(0x41559eb73d2a493d18e8e956c17c77fc874e980ffc);

    // ns提币地址
    address internal ns_draw_address = address(0x419aaff826a7b85910800f99f900519d15451a0aa8);
...
contract nsMemberShare is Common {
    ...
    function userGetShare() public payable returns (uint) {
        require(msg.value >= 15 * 10 ** 6, "trx is not enough!");
        ...
        TRC20 token = TRC20(ns_contract);
        assert(token.transferFrom(ns_draw_address, msg.sender, poolShare.amount) == true);
        poolShare.status = 2;
        return SUCCESS;
    }
}
