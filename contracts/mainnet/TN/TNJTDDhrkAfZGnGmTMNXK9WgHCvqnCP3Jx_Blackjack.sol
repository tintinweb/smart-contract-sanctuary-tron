...
contract Blackjack {
     ...
     * Player can bet up to 5,000 TRX if there is enough liquidity.
     * 
     * @return uint256
     */
    ...
    function blockHashForRNGProduced(uint256 future_block_number) external view returns (bool) {
        return future_block_number < (block.number - 1);
    }
     ...
     * 
     * @param wager_amount Player's bet amount in TRX.
     * @param future_block_number This block number is used to check if the 
     * future block has been produced.
    ...
    (
        ...
        require(
            future_block_number < (block.number - 1), 
            "Please wait for future block to be produced."
        );
        
        // random bytes
        bytes32 random_bytes = keccak256(abi.encodePacked(blockhash(block.number - 1)));
    ...
    function generateRandomBytesAll(
        ...
        bytes32 random_bytes = keccak256(abi.encodePacked(blockhash(block.number - 1))); // random bytes
        bytes8[] memory playing_hand_hashes = getPlayingHandHashes();
        bytes8 data_hash;
        uint256 counter;
