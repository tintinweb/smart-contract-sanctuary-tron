...
//SourceUnit: MultiSigFundRaiser.sol

pragma solidity 0.8.6;

import "./EnumerableSet.sol";
import "./BaseMultiSigWallet.sol";
import "./TrxReceiver.sol";


/// @title
contract MultiSigFundRaiser is BaseMultiSigWallet, TrxReceiver {
    using EnumerableSet for EnumerableSet.AddressSet;

    event Redeem(address indexed to, uint amount);

    uint public redeemedAmount;
    ...
    function proposeRedeem(address payable to, uint amount) public returns (uint transactionId) {
        require(to != address(0), "can not send to zero address");
        require(getBalance() >= amount, "not enough trx");
        bytes memory data = abi.encodeWithSelector(this.redeem.selector, to, amount);
        return _submitTransaction(data);
    }

    /// @dev Allows to send trx to user. Transaction has to be sent by wallet.
    /// @param to Address of issuer.
    /// @param amount Amount to send to the issuer.
    function redeem(address payable to, uint amount)
    public
    onlyWallet
    {
        require(to != address(0), "can not send to zero address");
    ...
    function decodeCustomMethod(bool executed, bytes calldata data) public pure returns (TransactionDetail memory detail) {
        ...
        if (selector == this.redeem.selector) {
            ...
            detail.method = "redeem(address payable to, uint amount)";
            detail.receiver = to;
...
//SourceUnit: TrxReceiver.sol

pragma solidity 0.8.6;

contract TrxReceiver {
    ...
    function _receive() internal {
        require(msg.tokenvalue == 0, "only trx is allowed");
