...
// File: contracts\interfaces\IDaoRouter.sol

pragma solidity >=0.6.2;

interface IDaoRouter {
...
// File: contracts\interfaces\IDaoPair.sol
...
// File: contracts\libraries\DaoLibrary.sol
...
library DaoLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'57fd4cc6a63f1009fedf3ab9c32e06923f55890349d228330ca97ce2387447b5' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IDaoPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
// File: contracts\DaoRouter.sol

pragma solidity >=0.6.6;


contract DaoRouter is IDaoRouter {
    using SafeMath for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'DaoRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (IDaoFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IDaoFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = DaoLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = DaoLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'DaoRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = DaoLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'DaoRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = DaoLibrary.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IDaoPair(pair).mint(to);
    }
    function addLiquidityTRX(
        address token,
        uint amountTokenDesired,
        ...
        address pair = DaoLibrary.pairFor(factory, token, WTRX);
    ...
    function removeLiquidity(
        ...
        address pair = DaoLibrary.pairFor(factory, tokenA, tokenB);
        IDaoPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IDaoPair(pair).burn(to);
        (address token0,) = DaoLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'DaoRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'DaoRouter: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityTRX(
        address token,
        uint liquidity,
    ...
    function removeLiquidityWithPermit(
        ...
        IDaoPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IDaoPair(DaoLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = DaoLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'DaoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = DaoLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'DaoRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactTRXForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WTRX, 'DaoRouter: INVALID_PATH');
        amounts = DaoLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'DaoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWTRX(WTRX).deposit{value: amounts[0]}();
        assert(IWTRX(WTRX).transfer(DaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactTRX(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WTRX, 'DaoRouter: INVALID_PATH');
        amounts = DaoLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'DaoRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WTRX, 'DaoRouter: INVALID_PATH');
        amounts = DaoLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'DaoRouter: EXCESSIVE_INPUT_AMOUNT');
        IWTRX(WTRX).deposit{value: amounts[0]}();
        assert(IWTRX(WTRX).transfer(DaoLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust trx, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = DaoLibrary.sortTokens(input, output);
            IDaoPair pair = IDaoPair(DaoLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'DaoRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactTRXForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        assert(IWTRX(WTRX).transfer(DaoLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'DaoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return DaoLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return DaoLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return DaoLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return DaoLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return DaoLibrary.getAmountsIn(factory, amountOut, path);
    }
}