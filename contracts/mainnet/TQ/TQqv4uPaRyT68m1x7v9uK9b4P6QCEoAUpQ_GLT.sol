...
contract GLT is TRC20 {
   ...
   function burn(uint256 value) external onlyOwner {
        require(balanceOf[msg.sender] >= value, "GLT: INSUFFICIENT_FUNDS");
  ...
  function sendCoin(address _to, uint _amount) public returns (bool sufficient) {
   ...
   function addBlockList(address wallet) onlyOwner public returns (bool) {
    ...
    function isBlockListed(address wallet) onlyOwner public view returns (bool) {
    ...
    function mint(address to, uint256 value) external onlyOwner {
        require(!stopmint, "GLT: MINT_ALREADY_STOPPED");

        _mint(to, value);
    }

    function stopMint() external onlyOwner {
        require(!stopmint, "GLT: MINT_ALREADY_STOPED");
