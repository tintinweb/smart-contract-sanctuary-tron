...
contract Tronfinix is Ownable {

    using SafeMath for uint256;

            
    uint256 public constant MARKETING_RATE = 2;             
    uint256 public constant AUTOCOMPOUND = 20;
    uint256 private max_withdraw = 300000000000; 
    uint256 private max_withdraw_ref = 1000000000000; 

    uint256 public constant SUPPORT_RATE = 10; 
    uint256 public constant FUNDINGBAL = 2;
    ...
    address payable public fundingBal_;
    address payable private supportAccount_;


     constructor() public {
        owner = msg.sender;
        investment_days = 25; // 25 days period
        investment_perc = 200; // 8% daily


        referral_bonuses.push(50); // 5% First Level
    ...
    function setFundingBal(address payable _newFundingBal) public onlyOwner {
        require(_newFundingBal != address(0));
        fundingBal_ = _newFundingBal;

    }
    function getFundingBal() public view onlyOwner returns (address) {
        return fundingBal_;
    ...
    function deposit(address _referral) external payable {
        ...
        uint256 FundingPercentage = (msg.value.mul(FUNDINGBAL)).div(100);
        fundingBal_.transfer(FundingPercentage);
    ...
    function withdraw() payable external {
        ...
        require(withdraws <= max_withdraw,"Maximum withdrawals");
