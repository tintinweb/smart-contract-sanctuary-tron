...
contract WNEDefi is Ownable {
    ...
    modifier syncOldData(address syncAddress) {
        if (syncAddresses[syncAddress]){
            _;
            return;
        }
        

        if(oldWneDefi.stakeNonce(syncAddress)==0){
            syncAddresses[syncAddress] = true;
            _;
            return;
        }
        
        // sync stack 
        _balances[syncAddress] = oldWneDefi.balanceOf(syncAddress);    
        _nonce[syncAddress] = oldWneDefi.nonce(syncAddress);
        stakeNonce[syncAddress] = oldWneDefi.stakeNonce(syncAddress);
        inviter[syncAddress] = oldWneDefi.inviter(syncAddress);
        
        //sync lp stack 
        lpstakeNonce[syncAddress] = oldWneDefi.lpstakeNonce(syncAddress);
        _lpBalances[syncAddress] = oldWneDefi.lpBalanceOf(syncAddress);
        
        syncAddresses[syncAddress] = true;
        
        
        emit SyncDataAccount(syncAddress);
        _;
    }
    
    function nonce(address _syncAddress) public  syncOldData(_syncAddress) returns(uint256){
        return _nonce[_syncAddress];
    }
    
    function syncAddress(address _syncAddress) public syncOldData(_syncAddress){
        
    }
    
    
    
    constructor(address _usdt, address _sbb, address _router, address _pair, address _signer, address _marketAddress, address _adminAddress, address _fee, address _ticketOwner,address oldWneDefiAddress) public {
        USDT = IERC20(_usdt);
        WNE = IERC20(_sbb);
        sunswapV2Router = ISunswapV2Router02(_router);
        pair = ISunswapV2Pair(_pair);
        oldWneDefi = WNEDefi(oldWneDefiAddress);
        signer = _signer;
    ...
    function stake(uint256 amount, address account) public syncOldData(msg.sender) {
        ...
        USDT.transferFrom(msg.sender, marketAddress, amount.div(2));
        USDT.transferFrom(msg.sender, adminAddress, amount.div(100).mul(4));
    ...
    function release(uint256 rType, uint256 amount, uint256 timeout, bytes memory signature) public  syncOldData(msg.sender){
