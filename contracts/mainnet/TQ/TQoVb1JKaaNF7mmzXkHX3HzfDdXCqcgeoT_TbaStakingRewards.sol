...
contract TbaStakingRewards is RewardsDistributionRecipient {
    ...
    function stake(address recommneder, uint256 amountLp, uint256 amountSeut) external updateRecommender(recommneder) updateReward(msg.sender) {
        ...
        if(0 < amountLp){
            ...
            if(0 == withdrawTime[msg.sender][0]){
                withdrawTime[msg.sender][0] = block.timestamp.add(withdrawCycle);
            }
        ...
        if(0 < amountSeut){
            ...
            if(0 == withdrawTime[msg.sender][1]){
                withdrawTime[msg.sender][1] = block.timestamp.add(withdrawCycle);
            }
        }
    }
    ...
    function getReward(uint i) public  updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            if(!isOthers(msg.sender)){
                ...
                require(block.timestamp > withdrawTime[msg.sender][i], 'It is not time to getReward the pledge');
